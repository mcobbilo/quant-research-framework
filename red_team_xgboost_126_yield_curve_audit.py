...
def run_xgboost_audit(seed=0, noise_std=0.0, guillotine=False, test_name="BASELINE"):
    ...
    df_returns = df_core[['ds', 'vustx_close', 'gld_close', 'hg_close', 'vix_close', 't10y2y', 'skew_zscore_252', 'vix_vxv_ratio_z', 'sent_0dte_repression_z']].copy()
    ...
    # Yield Curve Convexity Extraction
    df_returns['t10y2y_prev'] = df_returns['t10y2y'].shift(1)
    df_returns['t10y2y_mom_63'] = df_returns['t10y2y_prev'] - df_returns['t10y2y_prev'].shift(63)
    df_returns['t10y2y_mom_63'].fillna(method='bfill', inplace=True)
    df_returns['t10y2y_prev'].fillna(method='bfill', inplace=True)
    ...
    for idx, row in df_cont.iterrows():
        ...
        # Yield Curve Convexity Veto
        yc_convexity_veto = (row['t10y2y_prev'] < 1.0) and (row['t10y2y_mom_63'] > 0.40)
        ...
        if dr_copper_veto or yc_convexity_veto:
