...
def run_real_backtest():
    ...
    dataset = TimeSeriesDataSet.from_dataset(
        training, df, min_prediction_idx=target_idx
    )
    ...
    eval_df["Prev_Position"] = (
        eval_df["Position"].shift(1).fillna(1.0)
    )  # Assume it carried over LONG from Dec
    changes = eval_df[eval_df["Position"] != eval_df["Prev_Position"]]
    pos_map = {1.0: "LONG SPY", -1.0: "SHORT SPY", 0.0: "CASH"}

    print(
        f"{eval_df['Date'].iloc[0].strftime('%Y-%m-%d')} Initial Carry-Over -> {pos_map.get(eval_df['Position'].iloc[0])}"
    )

    for _, row in changes.iterrows():
        print(
            f"{row['Date'].strftime('%Y-%m-%d')} Switched to {pos_map.get(row['Position'])} (Prediction vector: {row['Forward_21d_Prediction']:.3f})"
        )
