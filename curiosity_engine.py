...
def get_database_schema():
    try:
        ...
        return "Standard Market Columns"
...
def execute_and_evaluate(code, hyp_name):
    with open("temp_hypothesis.py", "w") as f:
        f.write(code)
    try:
        result = subprocess.run(
            [sys.executable, "temp_hypothesis.py"],
            capture_output=True,
            text=True,
            timeout=120,
        )
        ...
        for line in result.stdout.split("\n"):
            if "RESULT_YIELD:" in line:
                y_val = float(line.split(":")[1].strip())
            if "RESULT_SHARPE:" in line:
                s_val = float(line.split(":")[1].strip())

        if y_val == 0.0 and s_val == 0.0:
            return {"status": "failure", "reason": "No metrics detected."}
...
DEEP_RESEARCH_PATH = (
    "/Users/milocobb/.gemini/antigravity/skills/deep-research/scripts/research.py"
)
try:
    from src.core.nodes.google_search import search_web  # Local search bridge

    OSINT_NODE = OSINTNode(
        HARNESS_CONFIG,
        call_llm,
    ...
    try:
        ...
        for name, m_df in mocks.items():
            ...
            if res["status"] == "success" and res["sharpe"] < -2.0:
                print(
                    f"[Synthetic Check] WARNING: Strategy had extreme negative Sharpe on {name}: {res['sharpe']}"
                )
        ...
        if os.path.exists(backup_db):
            shutil.move(backup_db, db_path)


def run_research_sequence(
    schema, iteration, champion_sharpe=-1.0, champion_code=None, mode="research"
):
    memory = read_memory()
    ...
    for attempt in range(2):
        ...
        print(
            f"[ACA] Sequence {iteration} Attempt {attempt + 1} FAILED. Invoking Shift-Right Analyzer..."
        )
        ...
        code = CODER_NODE.execute(
            strat["pitch"],
            schema,
            failure_msg=f"Previous Error: {reason}",
            knowledge=f"{knowledge}\n- NEW: {lesson}",
        )
...
def main_loop(max_iterations=10, mode="research"):
    ...
    for i in range(1, max_iterations + 1):
        ...
        result = run_research_sequence(
            schema, i, CHAMPION_SHARPE, CHAMPION_CODE, mode=mode
        )
        ...
        if result and result.get("status") == "success":
            ...
            if CHAMPION_SHARPE > -1.0 and new_sharpe < CHAMPION_SHARPE:
                ...
                print(
                    f"[REGRESSION] !! Sharpe dropped ({new_sharpe:.4f} < {CHAMPION_SHARPE:.4f}) !!"
                )
                perf_delta = (
                    (new_sharpe - CHAMPION_SHARPE) / CHAMPION_SHARPE
                    if CHAMPION_SHARPE != 0
                    else -1.0
                )
                ANALYZER_NODE.execute(
                    result["code"],
                    f"REGRESSION: Sharpe dropped from {CHAMPION_SHARPE} to {new_sharpe}",
                    is_regression=True,
                    performance_delta=perf_delta,
                )
                write_memory(
                    f"[R] {hyp_name}: Regression (Discarded) | Sharpe: {new_sharpe}"
                )
            else:
            ...
            reason = (
                result.get("reason", "Unknown Failure") if result else "Node Failure"
            )
    ...
    if BEST_FAILED_CODE:
        ...
        if optim_sharpe > CHAMPION_SHARPE and CHAMPION_SHARPE > -1.0:
            print(
                f"[EGGROLL-CHAMPION] >> ES Recovered a Champion! New Peak: {optim_sharpe:.4f} <<"
            )
            with open(
                os.path.join(VICTORIES_DIR, f"EGGROLL_Rescued_{int(time.time())}.py"),
                "w",
            ) as f:
                f.write(optim_code)
        elif optim_sharpe > BEST_FAILED_SHARPE:
            print(
                f"[EGGROLL] Partial Success: Improved from {BEST_FAILED_SHARPE} to {optim_sharpe}"
            )
            with open(
                os.path.join(VICTORIES_DIR, f"EGGROLL_Improved_{int(time.time())}.py"),
                "w",
            ) as f:
                f.write(optim_code)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--mode", type=str, default="research", choices=["research", "sweep"]
    )
