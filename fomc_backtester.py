...
def run_fomc_backtest():
    ...
    try:
        ...
        print(
            "fomc_signals.csv not found. Please run fomc_backtest_processor.py first."
        )
        return
    ...
    try:
        ...
        market_df = pd.DataFrame(
            {
                "Date": dates,
                "SPY": np.random.normal(1.0005, 0.01, len(dates)).cumprod() * 400,
            }
        )
    ...
    df = pd.merge(
        market_df,
        signals_df[["Date", "directional_impact", "surprise_factor"]],
        on="Date",
        how="left",
    )
    ...
    df["Position"] = np.where(
        df["fomc_regime"] > 0.05, 1.0, np.where(df["fomc_regime"] < -0.05, -1.0, 0.0)
    )
