...
def train_tft_for_asset(df_train, df_test, features, target_col, max_epochs=5):
    ...
    preds = tft.predict(val_dataloader, return_y=False)
...
def run_trifecta_audit(seed=42, noise_std=0.0, guillotine=False, test_name=""):
    ...
    features = [
        ...
        'nyhgh_nylow_10d', 'vix_tnx_pct_200', 'vix_tnx_tsi', 'vix_tsi', 'vix_vvix_ratio', 'spy_ppo_signal',
    ...
    df_core[features] = df_core[features].replace([np.inf, -np.inf], np.nan).ffill().bfill().fillna(0)

    if noise_std > 0:
        np.random.seed(seed)
        for col in features:
            std_dev = df_core[col].std()
            df_core[col] = df_core[col] + np.random.normal(0, std_dev * noise_std, len(df_core))
    ...
    for i in tqdm(range(total_steps)):
        ...
        for asset in ['spy', 'vustx', 'gld', 'vix']:
            ...
            # --- 1. LightGBM (30%) ---
            clf_lgb = lgb.LGBMClassifier(n_estimators=100, max_depth=3, learning_rate=0.05, random_state=seed, verbose=-1, n_jobs=1)
            clf_lgb.fit(X_train, y_train)
            prob_lgbm = clf_lgb.predict_proba(X_test)[0][1]
            
            # --- 2. XGBoost (15%) ---
            clf_xgb = xgb.XGBClassifier(n_estimators=100, max_depth=3, learning_rate=0.05, random_state=seed, subsample=0.8, n_jobs=1, eval_metric='logloss')
            ...
            try:
                ...
                raise NotImplementedError("Isolating LGBM/XGB components to bypass TFT memory corruption.")
...
if __name__ == "__main__":
    ...
    try:
        if mode == "seed0" or mode == "all":
            run_trifecta_audit(seed=0, noise_std=0.0, guillotine=False, test_name="SEED_0_PERTURBATION")
        if mode == "noise" or mode == "all":
            run_trifecta_audit(seed=42, noise_std=0.05, guillotine=False, test_name="5PCT_NOISE_INJECTION")
        if mode == "guillotine" or mode == "all":
            run_trifecta_audit(seed=42, noise_std=0.0, guillotine=True, test_name="DATA_GUILLOTINE_TIMETRAVEL")
    except Exception as e:
