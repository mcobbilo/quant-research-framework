...
def scrape_market_diet():
    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            page.goto("https://www.wsj.com/market-data/stocks/us", wait_until="domcontentloaded", timeout=60000)
            ...
            if advances == "0" and declines == "0":
                print("[Breadth Scraper] WARNING: Could not parse exact metrics from WSJ Market Diary plain text.")
    ...
    try:
        with open(filepath, 'r') as f:
            ...
            if len(lines) > 0 and lines[-1].split(",")[0].strip() == today_str:
                print(f"Skipping {os.path.basename(filepath)} - Today's date ({today_str}) already exists.")
                return
