...
def run_repair():
    ...
    for f in sorted(csv_files):
        ...
        print(f" -> Found {missing_count} missing days. Interpolating mathematically (Linear)...")
        ...
        with open(f, 'w') as fp:
            ...
            fp.write("      Date,       Open,       High,        Low,      Close,        Volume\n")
            
            # Formatter for proper padding
            for _, row in df.iterrows():
                fp.write(f"{row['Date']:10}, {row['Open']:10.3f}, {row['High']:10.3f}, {row['Low']:10.3f}, {row['Close']:10.3f}, {int(row['Volume']):12}\n")
