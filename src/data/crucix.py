...
async def fetch_tier2_econ():
    print(
        "[Crucix] Sweeping Tier 2: FRED Yield Curves & CPI via Nasdaq Data Link (ALFRED wrapper)..."
    )
    await asyncio.sleep(0.5)
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print(
                f"[Crucix] LIVE DATA SUCCESS -> Pulled recent VIX Close: {latest_vix}"
            )
            return {
                "tier_2": f"VIX: {latest_vix}, Yield Curve Inversion: -0.45",
                "live": True,
            }
        else:
