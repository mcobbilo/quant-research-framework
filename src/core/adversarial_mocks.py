...
def get_base_data():
    ...
    try:
        df = pd.read_sql_query(
            "SELECT * FROM core_market_table ORDER BY Date ASC", conn
        )
    except Exception:
        df = pd.DataFrame()
    conn.close()
...
def generate_adversarial_paths():
    ...
    if base_df.empty:
        print(
            "[WARNING] Could not load base market data, falling back to degenerate mocks."
        )
    ...
    if n_blocks > 0:
        blocks = [
            base_df.iloc[i * block_size : (i + 1) * block_size] for i in range(n_blocks)
        ]
        np.random.seed(42)  # Deterministic for consistent testing
        shuffled_idx = np.random.permutation(len(blocks))
        bootstrapped_df = pd.concat([blocks[i] for i in shuffled_idx]).reset_index(
            drop=True
        )
        # Keep Date strictly monotonically increasing
        bootstrapped_df["Date"] = base_df["Date"][: len(bootstrapped_df)].reset_index(
            drop=True
        )
    ...
    if len(fat_tail_df) > num_jumps:
        jump_indices = np.random.choice(
            fat_tail_df.index[1:-1], num_jumps, replace=False
        )
