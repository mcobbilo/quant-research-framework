...
class AnalyzerNode:
    ...
    def execute(self, code, logs, knowledge_file="KNOWLEDGE.md", is_regression=False, performance_delta=0.0):
        ...
        mission_str = f"Perform a REGRESSION ANALYSIS. The new hypothesis performed WORSE than the current champion by {performance_delta:.2%}. Identify why this refinement failed." if is_regression else "Perform a granular CAUSAL ANALYSIS of this failure."
...
"""
        ...
        prompt_epsilon = epsilon_template.format(code=code, logs=logs, performance_delta=performance_delta)
        msgs_epsilon = [{"role": "system", "content": epsilon_system}, {"role": "user", "content": prompt_epsilon}]
        
        knowledge_file = "KNOWLEDGE.md"
        analysis_raw = self.call_llm(msgs_epsilon, temperature=0.2, role_context="Epsilon")
        ...
        if analysis_raw:
            ...
            if os.path.exists(knowledge_file):
                with open(knowledge_file, "r") as f:
                    ...
                    if hashes: parent_hash = hashes[-1]
                    if indices: index = int(indices[-1]) + 1
