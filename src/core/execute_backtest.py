...
def execute_backtest():
    logging.info("[Backtest] Initiating Full-Timeline Walk-Forward Backtest on MULTI-ASSET Global Pipeline...")
    ...
    hist_exog = [
        'volume', 'high', 'low', 't10y2y', 
        'vix_bb_width', 'spy_vwap_20', 
        'nyhgh_nylow_10d', 'world_cb_liq', 'vix_tnx_pct_200',
        'vix_tnx_tsi', 'vix_tsi', 'vix_vvix_ratio', 'spy_ppo_signal',
        'vix_vvix_ratio_z', 'sent_crossasset_vol_ratio', 'sent_0dte_repression_z',
        'vix_vxv_ratio_z', 'spy_vol_ratio_21_252', 'spy_accel_mom',
        'spy_atr_14', 'spy_atr_pct', 'market_breadth_zscore_252d',
        'vix_to_10y_mom21', 'corr_spy_vustx_63', 'nya200r'
    ]
    
    df_model = df_panel[['unique_id', 'ds', 'y'] + hist_exog].dropna()
    logging.info(f"[Backtest] Target Matrix Size (Panel Data): {len(df_model)} Total Rows across 4 Assets")
    ...
    if 'xLSTM' in cv_df.columns:
        ...
        df_pred_pivot = df_pred_pivot.rename(columns={
            'cutoff': 'signal_date',
            'SPY': 'prob_up_spy',
            'VUSTX': 'prob_up_vustx',
            'GLD': 'prob_up_gld',
        ...
        print(f"-> Total Asset Return                 : {df_cont['cum_asset'].iloc[-1]:.2f}x")
        print(f"-> Total Multi-Asset Strategy Return  : {df_cont['cum_strat'].iloc[-1]:.2f}x")
        print(f"-> Asset Max Drawdown                 : {calculate_mdd(df_cont['asset_return']):.2%}")
        print(f"-> Strategy Max Drawdown              : {calculate_mdd(df_cont['strategy_return']):.2%}")
        
        initial_investment = 100000
        print(f"\n-> $100k invested in S&P 500 becomes  : ${initial_investment * df_cont['cum_asset'].iloc[-1]:,.2f}")
        print(f"-> $100k invested in Model becomes    : ${initial_investment * df_cont['cum_strat'].iloc[-1]:,.2f}")
        ...
        yearly = df_cont.groupby('year').apply(lambda x: pd.Series({
