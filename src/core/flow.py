...
from models.timesfm_wrapper import (
    TimesFM2_5,
...
class MarketForecastingFlow(Flow[MarketState]):
    ...
    def latent_projection_and_deliberation(self):
        ...
        for i in range(council_size):
            ...
            agent_latent = self.council.project_agent_reasoning(
                features, previous_consensus=current_consensus
            )
        ...
        print(
            f"[Flow] Self-Organized Consensus Score: {self.state.consensus_score:.4f}"
        )
    ...
    def execute_long_trade(self):
        ...
        self.state.final_action = (
            f"EXECUTED LONG SPY (Size: {trade_size:.2f})"
            if success
            else "BLOCKED BY GUARDS"
        )

        # Save to memory
        mem = LocalMemoryStore()
        mem.save_experiential_memory(
            run_id="run_101",
            model_params="TimesFM-200m",
            sharpe_ratio=0.0,
            rationale=rationale,
        )
