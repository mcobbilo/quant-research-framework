...
class JepaMcpServer:
    ...
    def get_market_context(self, ticker: str, start_date: str, end_date: str) -> Dict:
        ...
        """
        query = (
            "SELECT * FROM market_features WHERE ticker = ? AND date BETWEEN ? AND ?"
        )
            ...
            "nodes": [
                {
                    ...
                    "desc": "Regime-Aware GMM Attention Core",
                },
                {
                    "id": "Tools",
                    "type": "bridge",
                    "file": "src/core/mcp_data_server.py",
                    "desc": "MCP Data & Skill Interface",
                },
                {
                    "id": "Skills",
                    "type": "logic",
                    "file": "src/skills/base_skill.py",
                    "desc": "Modular Quant Mathematical Operations",
                },
                {
                    "id": "Auditor",
                    "type": "agent",
                    "file": "curiosity_engine.py",
                    "desc": "Agent Delta Self-Healing Loop",
                },
                {
                    "id": "Extractor",
                    "type": "pipeline",
                    "file": "src/experimental/jepa_extractor.py",
                    "desc": "Feature Engineering & 20D Backtest Fork",
                },
            ],
            "edges": [
                {
                    "source": "Auditor",
                    "target": "Brain",
                    "type": "stability_audit",
                    "desc": "Delta monitors model weights for gradient explosion",
                },
                {
                    "source": "Extractor",
                    "target": "Tools",
                    "type": "data_request",
                    "desc": "Backtest fork calls MCP for microstructure context",
                },
                {
                    "source": "Brain",
                    "target": "Tools",
                    "type": "skill_invocation",
                    "desc": "Attention head selects 'VolTargeting' skill",
                },
                {
                    "source": "Tools",
                    "target": "Skills",
                    "type": "bridge",
                    "desc": "MCP server serves standalone skills as tools",
                },
            ],
        }
        return graph
    ...
    def _detect_domains(self, columns: List[str]) -> Dict[str, List[str]]:
        ...
        domain_map = {
            "CREDIT": [
                c
                for c in columns
                if any(k in c for k in ["HYG", "LQD", "credit", "spread"])
            ],
            "VOLATILITY": [
                c
                for c in columns
                if any(k in c for k in ["VIX", "volatility", "ATR", "boll"])
            ],
            "BREADTH": [
                c
                for c in columns
                if any(k in c for k in ["NYA200R", "breadth", "advance_decline"])
            ],
            "MACRO": [
                c
                for c in columns
                if any(k in c for k in ["US10Y", "yield", "macro", "gravity"])
            ],
        }
        return domain_map
