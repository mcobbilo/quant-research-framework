...
logging.basicConfig(
    level=logging.INFO, format="%(asctime)s [%(levelname)s] HotReloader - %(message)s"
)
...
class LiveSelfModificationReloader:
    ...
    def run(self):
        self.start_celery()
        logging.info(
            f"Watching for Live Self-Modification events securely in {SRC_DIR}..."
        )
