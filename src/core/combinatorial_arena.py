...
DB_PATH = os.path.abspath(os.path.join(os.path.dirname(__file__), "..", "data", "market_data.db"))
...
def get_candidates():
    ...
    ignore_cols = ['Date', 'SPY_CLOSE', 'unique_id', 'ds', 'y'] + list(REAL_COLUMN_MAP.values())
...
def run_arena(fast_fail=True):
    ...
    logging.info(f"[Arena] Found {len(candidates)} potential candidate features for mutation.")
    ...
    markdown_lines.append(f"Tested mutating existing 9-feature baseline with `{len(test_subset)}` new SQLite candidates.")
    ...
    for r in results:
        ...
        markdown_lines.append(f"- {sym} **{r['mutation']}**: RMSE: {r['rmse']:.4f} (Diff vs Base: {r['improvement']:.4f})")
