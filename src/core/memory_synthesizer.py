...
class MemorySynthesizer:
    ...
    def synthesize(self) -> str:
        ...
        for _, reason in failures:
            r = reason.lower()
            if "nan" in r or "inf" in r: clusters["NAN_INSTABILITY"] += 1
            elif "future" in r or "shift(0)" in r: clusters["LOOKAHEAD_BIAS"] += 1
            elif "talib" in r or "pandas_ta" in r: clusters["BANNED_LIBRARY"] += 1
            elif "underperformed" in r: clusters["UNDERPERFORMANCE"] += 1
            else: clusters["DATA_ISSUE"] += 1

        # 3. Build the Synthesis Report
        report = "### MEMORY CONSOLIDATION REPORT\n"
        report += f"**Processed Entries**: {len(failures) + len(successes)}\n"
        report += f"**Top Success Pattern**: {successes[-1][0] if successes else 'None'}\n\n"
        ...
        if clusters["LOOKAHEAD_BIAS"] > 0:
            report += "- CAUTION: Avoid target calculation leaks. Verify T-1 compliance.\n"
