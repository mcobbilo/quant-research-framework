...
class EndogenousRoleAdapter(nn.Module):
    ...
    def forward(
        self, agent_latent: torch.Tensor, previous_consensus: torch.Tensor
    ) -> torch.Tensor:
...
class LatentCouncil:
    ...
    def __init__(
        ...
        feature_dim: int,
        latent_dim: int = 128,
        weights_path: str = "models/council_weights.pth",
    ):
    ...
    def project_agent_reasoning(
        self, agent_features: torch.Tensor, previous_consensus: torch.Tensor = None
    ) -> torch.Tensor:
        """
        ...
        feature_proxy = (
            agent_features[0, 0].item()
            if agent_features.dim() > 1
            else agent_features[0].item()
        )
    ...
    def calculate_consensus(self, agent_latents: List[torch.Tensor]) -> Dict:
        ...
        if not agent_latents:
            return {
                "consensus_score": 0.0,
                "mean_latent": torch.zeros((1, self.latent_dim)),
            }
        ...
        for i in range(len(agent_latents)):
            for j in range(i + 1, len(agent_latents)):
                sim = torch.nn.functional.cosine_similarity(
                    agent_latents[i], agent_latents[j]
                )
        ...
        return {
            ...
            "agent_deviations": [
                torch.norm(l - mean_latent).item() for l in agent_latents
            ],
        }

    def verify_prediction(
        self, predicted_latent: torch.Tensor, actual_market_features: torch.Tensor
    ) -> float:
        """
    ...
    def derive_action(
        self, latent_vector: torch.Tensor, consensus_score: float, vix: float = 20.0
    ) -> str:
        """
