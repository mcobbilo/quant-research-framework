...
def main():
    try:
        ...
        df_1d = pd.merge(df_1d, spy[['cutoff', 'cutoff_price']], on='cutoff', how='inner')
        
        df_1d['signal'] = np.where(df_1d['xLSTM-median'] > df_1d['cutoff_price'], 1, -1)
        df_1d['asset_return'] = (df_1d['y'] - df_1d['cutoff_price']) / df_1d['cutoff_price']
        ...
        yearly = df_1d.groupby('year').apply(lambda x: pd.Series({
