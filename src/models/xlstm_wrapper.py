...
class xLSTMForecast:
    ...
    def fit(self, df: pd.DataFrame):
        ...
        print(f"[{pd.Timestamp.now()}] [xLSTM] Initiating exponential gating pre-training over {len(df)} samples...")
        self.nf.fit(df=df)
    ...
    def cross_validation(self, df: pd.DataFrame, n_windows: int = 3, step_size: int = 1):
        """
        ...
        print(f"[{pd.Timestamp.now()}] [xLSTM] Commencing {n_windows}-window Cross Validation...")
        cv_df = self.nf.cross_validation(df=df, n_windows=n_windows, step_size=step_size)
        return cv_df
