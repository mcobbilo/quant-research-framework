...
class HybridJamba(nn.Module):
    def __init__(self, d_model: int, num_layers: int, attn_layer_offset: int = 7, num_classes: int = 5):
        super().__init__()
...
class JambaBridge:
    ...
    def _init_model(self, frame):
        ...
        return HybridJamba(d_model=32, num_layers=4, num_classes=5).to(self.device).eval()

    def _load_weights(self, frame):
        # Weights located in hst_framework relative to project sibling
        root_dir = os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
        base_dir = os.path.dirname(root_dir)
        ckpt_path = os.path.join(base_dir, "hst_framework", "models", "checkpoints", f"hst_alpha_cluster_frame_{frame}.pt")
        ...
        print(f"[JambaBridge] Successfully loaded weights from Frame {frame} on {self.device.type.upper()}.")
    ...
    def _get_live_tape(self, lookback=16):
        ...
        return torch.tensor(feature_matrix.T, dtype=torch.float32).unsqueeze(0).to(self.device), df['VIX'].iloc[-1]
    ...
    def get_latest_alpha_signal(self):
        with torch.no_grad():
            ...
            macro_emb = self.macro_proj(X_m.permute(0, 2, 1)).permute(0, 2, 1).contiguous()
            b, t, n, f = X_o.shape
            options_flat = X_o.view(b, t, -1)
            option_emb = self.options_proj(options_flat.permute(0, 2, 1)).permute(0, 2, 1).contiguous()
            
            # Repeat to align time-dimensions
            if option_emb.size(1) < macro_emb.size(1):
                option_emb = option_emb.repeat_interleave(macro_emb.size(1)//option_emb.size(1), dim=1)
                
            fused = torch.cat([macro_emb, option_emb], dim=-1)
            logits = self.model(fused)[:, -1, :] # Last entry
            probs = torch.softmax(logits, dim=-1)[0].cpu().numpy()
            
            regimes = ["Massive Left-Tail", "Negative Bias", "Neutral/Flat", "Positive Bias", "Massive Right-Tail"]
