...
class TimesFM2_5:
    def __init__(self, weights_dir="./timesfm_weights", context=512, horizon=10):
        ...
        if os.path.exists(self.weights_dir):
            print(
                f"[TimesFM] SUCCESS: Linked local HuggingFace weights at: {self.weights_dir}"
            )
            self.weights_loaded = True
        else:
            print("[TimesFM] WARNING: Weights directory missing.")
            self.weights_loaded = False

    def predict_quantiles(self, context_tensor):
        print(
            f"[TimesFM] Forward pass inferencing {self.horizon}-day horizon on {self.device}..."
        )
