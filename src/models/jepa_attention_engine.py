...
class JepaAttentionEngine(nn.Module):
    ...
    def __init__(self, domain_dims: dict, latent_dim: int = 16, num_heads: int = 4):
        ...
        self.encoders = nn.ModuleDict({
            dom: DomainEncoder(domain_dims[dom], latent_dim) for dom in self.active_domains
        })
        ...
        self.predictor_t5 = nn.Sequential(nn.Linear(16, 32), nn.ReLU(), nn.Linear(32, 16))
        self.predictor_t10 = nn.Sequential(nn.Linear(16, 32), nn.ReLU(), nn.Linear(32, 16))
        self.predictor_t20 = nn.Sequential(nn.Linear(16, 32), nn.ReLU(), nn.Linear(32, 16))
        ...
        self.regime_priors = nn.Parameter(torch.zeros(self.num_regimes, self.num_domains))
    ...
    def forward(self, domain_inputs: dict, return_regime=False):
        ...
        if not domain_blobs:
            return torch.zeros((1, 19), device=device), {}, torch.zeros((1, 1, 1), device=device)
        ...
        try:
            ...
            divergence = torch.cosine_similarity(credit_latent, breadth_latent, dim=1).unsqueeze(1)
            ...
            jepa_state = torch.cat([latent_context, divergence, entropy_raw, asymmetric_raw], dim=1)
        except (ValueError, KeyError, IndexError) as e:
            # Fallback if domains are missing (e.g. in synthetic tests)
            # Pad to 19D
            jepa_state = torch.cat([latent_context, torch.zeros((x.size(0), 3), device=x.device)], dim=1)
...
class PPOExecutionPipeline:
    ...
    def __init__(self, jepa_engine: JepaAttentionEngine, device='cpu'):
        ...
        self.jepa_optimizer = torch.optim.AdamW(self.jepa.parameters(), lr=1e-3, weight_decay=1e-4)
        self.actor_optimizer = torch.optim.AdamW(self.policy.parameters(), lr=1e-3, weight_decay=1e-4)
    ...
    def fit(self, domain_inputs: dict, fwd_returns: np.ndarray, epochs=30):
        ...
        # =========================================================================
        print(f"   [J-EPA] Bootstrapping Multi-Scale World Model ({epochs//2} Epochs)...")
        self.jepa.train()
        for epoch in range(epochs//2):
            ...
            loss_t5 = nn.MSELoss()(predictions['t5'][:-5], final_state[5:, :16].detach()) if t_max > 5 else torch.tensor(0.0, device=self.device)
            loss_t10 = nn.MSELoss()(predictions['t10'][:-10], final_state[10:, :16].detach()) if t_max > 10 else torch.tensor(0.0, device=self.device)
            loss_t20 = nn.MSELoss()(predictions['t20'][:-20], final_state[20:, :16].detach()) if t_max > 20 else torch.tensor(0.0, device=self.device)
            ...
            if epoch == 0 or epoch == (epochs//2) - 1:
                print(f"      - Epoch {epoch+1}/{epochs//2} | J-EPA MSE: {jepa_loss.item():.4f} | Regime Entropy: {regime_diversity_loss.item():.4f}")
        ...
        # =========================================================================
        print(f"   [PyTorch PPO] Executing Abstract Allocation Control ({epochs//2} Epochs)...")
        self.jepa.eval()
        self.policy.train()
        ...
        for epoch in range(epochs//2):
            ...
            action_spy_prob = torch.sigmoid(mu / (sigma + 1e-6))
            ...
            slippage = torch.cat([torch.tensor([0.0], device=self.device), action_deltas]) * 0.001
            ...
            port_returns = (action_spy * spy_returns) + ((1.0 - action_spy) * vustx_returns) - slippage
            
            # PyTorch Trajectory Computation (Sortino)
            mean_ret = port_returns.mean() * 252
            downside = torch.where(port_returns < 0, port_returns, torch.tensor(0.0, device=self.device))
            # HARDENING: Increased epsilon to 1e-4 for numerical stability during regime shifts
            vol = torch.sqrt(torch.mean(downside**2) + 1e-4) # 1e-4 is safer than 1e-8 for small windows
            ...
            if epoch == 0 or epoch == (epochs//2) - 1:
                print(f"      - Epoch {epoch+1}/{epochs//2} | RL Target Sortino: {sortino.item():.4f} | T-Dist NLL: {nll_loss.item():.4f}")
    ...
    def predict_proba(self, domain_inputs: dict, num_samples=10):
        ...
        with torch.no_grad():
            ...
            uncertainty_penalty = torch.clamp(std_mu / (mean_mu.abs() + 1e-4), 0.0, 1.0)
            base_allocation = (mean_mu > 0.0).float()
...
def _test_prototype_architecture():
    ...
    jepa = JepaAttentionEngine(domain_dims=domain_dimensions, latent_dim=16, num_heads=4)
    ...
    jepa = JepaAttentionEngine(domain_dims=domain_dimensions, latent_dim=16, num_heads=4).to(device)
    ...
    print("[COMPUTE] Pushing physical Forward Pass (Tensors -> Encoders -> Attention -> Predictive State)...")
    final_state, _, attn_weights = jepa(fake_inputs)
    
    print(f"\n✅ Pipeline Structural Integrity Validated:")
    print(f"-> Extracted Latent State Dimension: {final_state.shape} | Expected (32, 19)")
    print(f"-> Attention Matrix Weight Dimensions: {attn_weights.shape} | Expected (32, 7, 7) Phase 13 Spatial Rollback")
    print(f"-> Attention Self-Referential Sum (Dim 1 Check): {attn_weights[0, 0].sum().item():.2f} (Must == 1.0)")
    ...
    print("[WARN] Bypassing localized XGBoost integration test to prevent MacOS OpenMP segmentation fault.")
