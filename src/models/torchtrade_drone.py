        ...
        while retries <= self.max_retries:
            try:
                ...
                if retries > self.max_retries:
                    print(f"{RED}[Guardian] Max retries exceeded: {error_str}. Escalating.{RESET}")
                    raise
                
                # Classify the error
                if "rate limit" in error_str.lower() or "429" in error_str:
                    print(f"{YELLOW}[Guardian] Rate Limit detected. Applying exponential backoff...{RESET}")
                    time.sleep(2 ** retries)
                elif "timeout" in error_str.lower():
                    print(f"{YELLOW}[Guardian] Timeout detected. Retrying with jitter...{RESET}")
                    time.sleep(random.uniform(1.0, 3.0))
                else:
                    print(f"{YELLOW}[Guardian] Unexpected error '{error_str}'. Retrying ({retries}/{self.max_retries})...{RESET}")
                    time.sleep(1)
...
class RecallMaxEngine:
    ...
    def compress_history(self):
        ...
        print(f"{CYAN}[RecallMax] Compressing {len(self.history)} turns into dense token history...{RESET}")
        compressed_summary = {
...
def build_torchtrade_environment():
    print(f"{CYAN}[TorchTrade Engine] Reconstructing Offline Multi-Timeframe Matrix...{RESET}")
    ...
    print(f"{CYAN}[TorchTrade Engine] Formatting for Native PyTorch RL (TorchRL) ingestion...{RESET}")
    ...
    config = SequentialTradingEnvConfig(
        ...
        window_sizes=[5, 4, 3], # Give the neural network context arrays of 5 Days, 4 Weeks, and 3 Months!
        execute_on="1D",
    ...
    try:
        ...
        print(f"{RED}[TorchTrade Engine] Error Initializing Sequential Environment: {e}{RESET}")
        return None
...
def execute_torchtrade_drone():
    ...
    print(f"{CYAN}[TorchTrade Engine] Initializing Environment with Guardian Protections...{RESET}")
    env = guardian.execute(build_torchtrade_environment)
    
    if not env:
        return "ERROR: TorchTrade Init Failed"
        
    print(f"{MAGENTA}[TorchTrade Agent] Querying Local/LLM Policy Ensemble for Zero-Shot evaluation...{RESET}")
    try:
        decision = guardian.execute(execution_step, env)
        
        # Log to memory
        recall_engine.log_turn("Multi-timeframe SPY matrix", decision, "Execution pending")
