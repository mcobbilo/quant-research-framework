...
def generate_all_charts():
    ...
    for section in sections[1:]:
        ...
        temp_df = df.dropna(subset=[var_name, 'SPY_CLOSE', 'SPY_OPEN', 'SPY_HIGH', 'SPY_LOW'])
        if temp_df.empty:
            new_md_content.append('### ' + section)
            continue
        ...
        ax1.bar(up.index, up['SPY_CLOSE'] - up['SPY_OPEN'], bottom=up['SPY_OPEN'], color='#2ca02c', width=0.6)
        ax1.vlines(up.index, up['SPY_LOW'], up['SPY_HIGH'], color='#2ca02c', linewidth=1)
        ax1.bar(down.index, down['SPY_CLOSE'] - down['SPY_OPEN'], bottom=down['SPY_OPEN'], color='#d62728', width=0.6)
        ax1.vlines(down.index, down['SPY_LOW'], down['SPY_HIGH'], color='#d62728', linewidth=1)
        
        ax1.plot([], [], color='white', label='SPY Price (OHLC)')
        trigger_color = 'white' if direction == 'min' else 'lime'
        ax1.axvline(x=ext_date, color=trigger_color, linestyle='--', linewidth=2.5, label='Extreme Parity Trigger')
        
        local_idx = window_df.index.get_loc(ext_date)
        if local_idx + 60 < len(window_df):
            day_60 = window_df.index[local_idx + 60]
            ax1.axvline(x=day_60, color='green', linestyle=':', linewidth=2.5, label='60-Day Forward Parity')
            p_in = window_df.iloc[local_idx]['SPY_CLOSE']
            p_out = window_df.iloc[local_idx + 60]['SPY_CLOSE']
            ret = (p_out / p_in) - 1
            ax1.annotate(f"Forward 60D:\n{'+' if ret>0 else ''}{ret*100:.2f}%", 
                         xy=(day_60, p_out), xytext=(day_60 + pd.Timedelta(days=5), p_out),
                         bbox=dict(boxstyle="round,pad=0.4", fc="#333333", ec="green", lw=1),
                         fontsize=10, color='white')
                         
        ax1.set_title(f'SPY Price Action Following {var_name} {"Crash" if direction=="min" else "Spike"}', fontsize=14)
        ax1.legend(loc='upper left')

        ax2.plot(window_df.index, window_df[var_name], color='orange' if direction=='max' else 'purple', linewidth=2, label=var_name)
        ax2.axvline(x=ext_date, color=trigger_color, linestyle='--', linewidth=2.5)
        
        ext_val = temp_df.loc[ext_date, var_name]
        ax2.annotate(f"Extreme: {ext_val:.2f}",
                     xy=(ext_date, ext_val),
        ...
        if f"insight{num}_chart.png" not in section and f"insight{num}_ohlc.png" not in section:
