...
def analyze_vix_term_structure():
    ...
    try:
        ...
        if isinstance(df.columns, pd.MultiIndex):
            ...
            close_cols = df.xs('Close', level='Price', axis=1) if 'Price' in df.columns.names else df['Close']
        else:
            close_cols = df
        ...
        for h in horizons:
            ...
            results[h] = {
                'normal_ev': normal_ev, 'normal_win': normal_win,
                'tail_ev': tail_ev, 'tail_win': tail_win
            }
        ...
        win_rate_desc = "despite lower overall win rates" if h5['tail_win'] < h5['normal_win'] else "alongside exponentially higher win rates"
        
        fact_str = f"Fact: VIX Backwardation (VIX > VIX3M) yields a {h5['tail_ev']:.2f}% Expected Value over 5 days (vs {h5['normal_ev']:.2f}% normal), " \
