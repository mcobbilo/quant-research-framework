...
def download_orthogonal_features(penalty_weight=49.0):
    ...
    df["Insight_Liquidity_Contraction"] = (
        df["Global_Liquidity_Velocity_21d"] < 0
    ).astype(int)
    ...
    df["Insight_FX_Shock"] = (
        (df["FX_DXY_Velocity_20d"] > 3.0) | (df["FX_Yen_Shock_5d"] < -4.0)
    ).astype(int)

    _safe_haven_sma = df["Safe_Haven_Ratio"].rolling(50).mean()
    df["Insight_Safe_Haven_Flight"] = (
        df["Safe_Haven_Ratio"] > _safe_haven_sma * 1.05
    ).astype(int)
...
def backtest_meta_labeler(penalty_weight=49.0):
    ...
    gmm_feats = [
        "BAMLC0A0CM",
        "VIX_BB_WIDTH",
        "SPY_RSP_MOMENTUM_60D",
        "Global_Liquidity_Velocity_21d",
    ]
    ...
    print(
        f"[Phase 133] 15-Year Market Regimes Segmented:\n{df['Macro_Regime'].value_counts()}\n"
    )
    ...
    for i in range(train_size + purge_period, len(df), 60):
        ...
        for r in range(3):
            ...
            if len(train_r) < 30:
                ...
                dtrain_r = xgb.DMatrix(
                    train_data[features], label=train_data["Target_Crash"]
                )
            else:
                dtrain_r = xgb.DMatrix(train_r[features], label=train_r["Target_Crash"])

            model_r = xgb.train(
                params, dtrain_r, num_boost_round=30, obj=asymmetric_logloss
            )
            models[r] = model_r
        ...
        for j in range(len(test_data)):
            ...
            portfolio_value = (
                capital
                + (shares_spy * price_spy)
                + (shares_vustx * price_vustx)
                + (shares_gld * price_gld)
            )
            ...
            if prob > 0.80:  # 🚨 ABSOLUTE PANIC TRIGGER 🚨
                ...
                vustx_valid = (today["VUSTX"] > today["VUSTX_SMA200"]) and (
                    today["CORR_SPY_VUSTX_63"] < 0.0
                )
                gld_valid = (today["GLD"] > today["GLD_SMA200"]) and (
                    today["CORR_SPY_GLD_63"] < 0.0
                )
                ...
                if vustx_valid and gld_valid:
                    ...
                    target_cash = (
                        portfolio_value  # Total system failure, retreat to 0.0 yield
                    )
            ...
            if (
                abs(delta_spy) > rebalance_threshold
                or abs(delta_vustx) > rebalance_threshold
                or abs(delta_gld) > rebalance_threshold
            ):
                # Liquidations first
                if delta_spy < 0:
                    capital -= delta_spy
    ...
    for year, group in results.groupby("Year"):
        meta_ret = (
            (group["Radar_Nav"].iloc[-1] / group["Radar_Nav"].iloc[0]) - 1.0
        ) * 100
        spy_ret = (
            (group["SPY_Price"].iloc[-1] / group["SPY_Price"].iloc[0]) - 1.0
        ) * 100
    ...
    print(
        f"{'Year':<6} | {'Meta-Labeler (%)':<18} | {'S&P 500 (%)':<16} | {'Alpha (%)':<10}"
    )
    print("-" * 65)
    ...
    # Absolute Totals
    meta_total = (
        (results["Radar_Nav"].iloc[-1] / results["Radar_Nav"].iloc[0]) - 1.0
    ) * 100
    spy_total = (
        (results["SPY_Price"].iloc[-1] / results["SPY_Price"].iloc[0]) - 1.0
    ) * 100
    ...
    print(
        f"XGBoost Black-Swan Radar    | Total Return: {meta_total:+.2f}% | Max DD: {meta_dd:,.2f}%"
    )
    print(
        f"Baseline SPY (Buy & Hold)   | Total Return: {spy_total:+.2f}% | Max DD: {spy_dd:,.2f}%"
    )
    print("=======================================================================\n")
    ...
    ax.set_title(
        f"Phase 125: Swarm Tri-Modal Defensive Allocation ({penalty_weight}x Alpha)",
        fontsize=16,
        fontweight="bold",
    )
