...
def main():
    ...
    print(
        f"[Full Backtest] Loaded {len(df)} operational business days. Attaching native algorithmic geometries..."
    )
    ...
    print("[Full Backtest] Initializing MetaStrategyClassifier (1.0x Hard Capped)...")
    ...
    print(
        f"S&P 500 (Buy & Hold) | Total Return: {spy_total_return:>8,.2f}% | Worst Drawdown: {worst_dd_spy:.2f}%"
    )
    print(
        f"Meta-Classifier      | Total Return: {strat_total_return:>8,.2f}% | Worst Drawdown: {worst_dd_strat:.2f}%"
    )
    print("===================================================================\n")
    ...
    for y in years:
        ...
        print(
            f"{y:<6} | {yr_ret_strat:>11.2f}% | {yr_ret_spy:>11.2f}% | {alpha:>11.2f}%"
        )
    ...
    with open(
        "/Users/milocobb/Desktop/Recent Swarm Papers/quant_framework/execution_tearsheet.md",
        "w",
    ) as f:
        f.write(md_output)
