...
def generate_chart():
    ...
    df = pd.read_sql(
        "SELECT Date, SPY_CLOSE, T10YFF FROM core_market_table", conn, index_col="Date"
    )
    ...
    ax1.axvline(
        ...
        linestyle="--",
        linewidth=2.5,
        label="Maximum Yield Inversion Trigger",
    )
    ...
    if local_idx_min + 60 < len(window_df):
        ...
        ax1.axvline(
            ...
            linestyle=":",
            linewidth=2.5,
            label="60 Trading-Day Forward Horizon",
        )
    ...
    ax1.set_title(
        "SPY Price Action Following Absolute T10YFF Maximum Inversion",
        fontsize=15,
        fontweight="bold",
    )
    ...
    ax2.plot(
        ...
        color="#d62728",
        linewidth=2,
        label="T10YFF (10Y Yield - Fed Funds)",
    )
    ax2.axhline(
        y=0,
        color="black",
        linestyle="-",
        linewidth=1.5,
        label="Inversion Parity Limit (0%)",
    )
    ...
    ax2.annotate(
        f"Max Inversion Penalty: {min_val:.2f}%",
        xy=(min_date, min_val),
