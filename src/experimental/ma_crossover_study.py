...
def analyze_ma_breakdowns():
    ...
    df['VIX_Close'] = yf.download('^VIX', start='1986-01-01', progress=False)['Close'].squeeze()
    ...
    waterfall_condition = (df['Below_All'] == True) & (df['Above_All_Recent_5D'] == 1.0)
    ...
    wf_vix = waterfall_events[['VIX_Close', 'Fwd_Ret_5D', 'Fwd_Ret_10D', 'Fwd_Ret_20D']].dropna()
    ...
    for event_name, stats in results.items():
        if event_name == 'Waterfall_VIX':
            print(f"--- Waterfall VIX Environment ---")
