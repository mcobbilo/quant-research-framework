...
import pandas as pd
import numpy as np
import os # Included for consistency with FRED API instructions, though not used here.
...
try:
    ...
    if 'Close' in df.columns:
        ...
        if isinstance(df.columns, pd.MultiIndex) and 'Close' in df.columns.get_level_values('Price'):
             df = df.xs('Close', level='Price', axis=1)
        else:
            raise ValueError(f"Could not find 'Close' column in downloaded data for {ticker}")
...
avg_return_200_crash = returns_200_crash.mean() if not returns_200_crash.empty else np.nan

# Convert average returns to percentage for clear comparison
avg_return_20_crash_pct = avg_return_20_crash * 100 if not np.isnan(avg_return_20_crash) else np.nan
avg_return_200_crash_pct = avg_return_200_crash * 100 if not np.isnan(avg_return_200_crash) else np.nan
