...
def get_ml_dataframe():
    ...
    df["Fwd_20D_Min_Price"] = (
        df["SPY_CLOSE"].shift(-1).rolling(window=20).min().shift(-19)
    )
...
def execute_xgboost_pipeline():
    ...
    y = (
        (ml_df["Fwd_20D_Return"] > 0.0) & (ml_df["Fwd_20D_Max_Drawdown"] > -0.05)
    ).astype(int)
    ...
    results_df = pd.DataFrame(
        {"Predicted_Probability": out_of_sample_preds}, index=out_of_sample_dates
    )
    ...
    results_df["Allocation"] = np.where(
        results_df["Predicted_Probability"] >= 0.55, 1.0, 0.0
    )

    results_df["XGBoost_Daily_Ret"] = (
        results_df["SPY_Daily_Ret"] * results_df["Allocation"]
    )
    ...
    ax.plot(
        results_df.index,
        results_df["Cumulative_XGBoost"],
        color="#00ff00",
        linewidth=2.5,
        label="XGBoost Walk-Forward Physics Engine",
    )

    ax.set_title(
        "Out-of-Sample XGBoost Allocation Trajectory (5-Year Rolling Train, 20-Day Step)",
        fontsize=16,
        color="white",
        pad=20,
    )
    ...
    plt.title(
        "XGBoost Macroeconomic Intelligence Decoder (Top 15 Absolute Features)",
        color="white",
    )
    # Change axis label colors
    # Ensure dark mode compliance for the SHAP graph
    plt.savefig(
        "/Users/milocobb/.gemini/antigravity/brain/86f8d6d6-545f-43de-8268-7b50b6d1c47a/shap_decoder_v7.png",
        dpi=120,
        bbox_inches="tight",
    )
    plt.close()
