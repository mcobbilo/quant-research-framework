...
def evaluate_drone():
    ...
    model_path = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "models", "ppo_quant_drone.zip")
    ...
    for year, row in yearly.iterrows():
        print(f"{int(year)} | {row['Drone_Yr_Ret']:>9.2%} | {row['SPY_Yr_Ret']:>8.2%} | {row['Drone_Max_DD']:>9.2%} | {row['SPY_Max_DD']:>8.2%}")
