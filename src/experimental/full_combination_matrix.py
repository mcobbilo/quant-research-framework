...
def run_combinations():
    ...
    manual_maps = {
        1: ('T10YFF', 'min'), 2: ('VIX_TNX_PCT_SMA_200', 'max'),
        3: ('MCO_PRICE', 'min'), 4: ('AD_LINE_PCT_SMA', 'min'),
        5: ('BAMLC0A0CM', 'max'), 6: ('SPY_PCT_SMA_20', 'max'),
        7: ('SPY_STOCH_5', 'min'), 8: ('T10Y2Y', 'min'),
        9: ('VIX_MOVE_SPREAD_5D', 'max'), 10: ('AD_LINE_10D_ROC', 'max')
    }
    ...
    for section in sections[1:]:
        ...
        if num in manual_maps:
            ...
            if 'Highest' in section or 'Top 50' in section or 'outpaces' in section:
                direction = 'max'
            elif 'Lowest' in section or 'Bottom 50' in section or 'collapses' in section:
                direction = 'min'
                
        if not var_name or not direction: continue
        if var_name == 'VVIX_PCT_SMA': var_name = 'VVIX_PCT_SMA_50'
    ...
    out_lines.append(f"Out of the {(len(insights) * (len(insights)-1)) // 2} possible brute-force pairings generated by the algorithm, these were the absolute active collision matrix outputs that historically occurred.\n")
    out_lines.append("---\n")
        ...
        for j in range(i + 1, len(nums)):
            ...
            if len(combo_df) > 0:
                ...
                if len(combo_df) >= 2 and (w60 == 100 or w20 == 100 or fwd60 >= 15.0):
                    ...
                    out_lines.append(f"**( `{var_i}` {dir_i.upper()} Extrema   AND   `{var_j}` {dir_j.upper()} Extrema )**")
                    out_lines.append(f"- **Occurrences:** {len(combo_df)}")
                    out_lines.append(f"- **Trigger Dates:** {', '.join([str(d.date()) for d in combo_df.index])}")
                    out_lines.append(f"- **5D Return:** `{fwd5:+.2f}%` *(Win Rate: {w5:.0f}%)*")
                    out_lines.append(f"- **10D Return:** `{fwd10:+.2f}%` *(Win Rate: {w10:.0f}%)*")
                    out_lines.append(f"- **20D Return:** `{fwd20:+.2f}%` *(Win Rate: {w20:.0f}%)*")
                    out_lines.append(f"- **60D Return:** `{fwd60:+.2f}%` *(Win Rate: {w60:.0f}%)*")
                    out_lines.append("\n---")
    ...
    print(f"Intersection sweep successfully resolved across 3,240 combinations.")
    print(f"Identified exactly {overlap_count} physical combinatorial edge occurrences.")
    print(f"Algorithm matrix generated successfully and saved directly to Brain Framework: {out_path}.")
