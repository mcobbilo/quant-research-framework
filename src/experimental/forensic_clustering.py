...
def run_clustering():
    ...
    print(f"[Forensics] De-duplicated into {len(distinct_panics)} distinctly isolated Macro events.\n")
    ...
    for i, t_date in enumerate(distinct_panics):
        ...
        print(f"==========================================================================")
        print(f"                      CRASH EVENT {i+1}: {t_date.strftime('%Y-%m-%d')}")
        print(f"==========================================================================")
        ...
        for r_idx, row in window.iterrows():
            ...
            day_str = f"Day T{day_offset:+} " if day_offset != 0 else ">> T+0 (TRIGGER) <<"
            ...
            print(f"{day_str:>19} | Sigma: {z:>5.2f} | VIX High: {vix_h:>5.2f} (Breached BB_Upper? {bb_breach:<3}) | PPO: {ppo:>6.2f}%")
        print("")
