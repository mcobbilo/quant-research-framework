...
def run_ppo_study():
    ...
    for index, row in df_sorted.iterrows():
        ...
        for t_date, _ in top_50:
            if (
                abs((index - t_date).days) < 15
            ):  # 15 calendar days buffer between macro events
                is_isolated = False
                break
        if is_isolated:
    ...
    print("\n=========================================================================")
    print("Top 50 Distinct Baseline PPO Spikes (25-Year History)")
    print(
        f"{'Date':<12} | {'VIX PPO_7':<10} | {'SPY Return (T+3)':<18} | {'SPY Return (T+5)':<18}"
    )
    print("-------------------------------------------------------------------------")
    ...
    for idx, r in top_50_chrono:
        ...
        print(
            f"{idx.strftime('%Y-%m-%d'):<12} | {ppo:>8.2f}%  | {ret3:>13.2f}%    | {ret5:>13.2f}%"
        )

    print("-------------------------------------------------------------------------")
    print(
        f"AVERAGE T+3 RETURN: {sum(avg_3d) / len(avg_3d):.2f}% (Win Rate: {win_3d / len(avg_3d) * 100:.1f}%)"
    )
    print(
        f"AVERAGE T+5 RETURN: {sum(avg_5d) / len(avg_5d):.2f}% (Win Rate: {win_5d / len(avg_5d) * 100:.1f}%)"
    )
    print("=========================================================================")
