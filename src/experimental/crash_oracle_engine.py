...
def train_crash_oracle():
    ...
    print(f"Total True CRASH states (>10% drop over 20D): {crash_count} ({(crash_count/total_count)*100:.2f}%)")
    
    if crash_count == 0:
        print("Mathematical anomaly: No crashes found in dataset. Halting.")
        return
        
    excluded_cols = ['Fwd_20D_Return', 'Fwd_20D_Max_Drawdown', 'Fwd_20D_Min_Price', 'SPY_Daily_Ret', 'Is_Crash']
    ...
    ax2 = ax1.twinx()
    ax2.fill_between(out_dates, 0, y_pred_bool, color='#ff0055', alpha=0.3, step='post', label='Oracle Isolation Flag')
    ax2.set_yticks([]) # Hide the 1/0 axis to keep it clean
    ax2.set_ylim(0, 5) # Compress the red bars downward slightly
    
    plt.title('Phase 18 Isolation Forest Oracle (Unsupervised Structural Detection)', color='white', fontsize=16)
    
    lines_1, labels_1 = ax1.get_legend_handles_labels()
    lines_2, labels_2 = ax2.get_legend_handles_labels()
    ax1.legend(lines_1 + lines_2, labels_1 + labels_2, loc='upper left', facecolor='black')
