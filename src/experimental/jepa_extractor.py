...
def categorize_features(feature_names):
    ...
    for c in feature_names:
        ...
        elif any(k in c for k in ['TNX', 'T10Y', 'REAL_YIELD', 'INFLATION', 'Monetary', 'FED', 'CentralBank']):
...
def jepa_walk_forward_pipeline():
    ...
    ml_df['Entropy_SPY'] = ml_df['SPY_Daily_Ret'].rolling(SEQ_LEN).apply(shannon_entropy, raw=True).fillna(0.0)

    # 3. VPIN-Style Microstructure Proxy (Hypothesis 0014)
    # Volatility / Volume ratio as a proxy for toxic liquidity
    ml_df['VPIN_Proxy'] = (ml_df['SPY_HIGH'] - ml_df['SPY_LOW']) / (ml_df['SPY_VOLUME'].rolling(5).mean() + 1e-8)
    ml_df['VPIN_Proxy'] = ml_df['VPIN_Proxy'].rolling(SEQ_LEN).rank(pct=True).fillna(0.5)
    ...
    ml_df['0013_Close_Rel_K'] = ((cl - klower) / (kupper - klower + 1e-8)).shift(1).fillna(0)
    
    # 5. Rolling XGB Asymmetric Confidence Signal
    ml_df['SPY_Fwd_10D_Ret'] = (1 + ml_df['SPY_Daily_Ret']).rolling(10).apply(np.prod, raw=True) - 1
    ml_df['SPY_Fwd_10D_Ret'] = ml_df['SPY_Fwd_10D_Ret'].shift(-10).fillna(0.0)
    ml_df['VUSTX_Fwd_10D_Ret'] = (1 + ml_df['VUSTX_Daily_Ret']).rolling(10).apply(np.prod, raw=True) - 1
    ...
    for t in range(xgb_train_size, len(ml_df), 20):
        ...
        X_train, y_train = ml_df[xgb_features].iloc[t_start:t_end], y_target_xgb[t_start:t_end]
        reg = xgb.XGBRegressor(n_estimators=50, max_depth=3, learning_rate=0.1, 
                               objective=asymmetric_mse, n_jobs=-1, random_state=42)
        reg.fit(X_train, y_train)
        t_next = min(t + 20, len(ml_df))
        ml_df.iloc[t:t_next, ml_df.columns.get_loc('XGB_Asymmetric_Signal')] = reg.predict(ml_df[xgb_features].iloc[t:t_next]) * 1.5

    # --- Feature Discovery & Domain Mapping ---
    excluded_keywords = ['OPEN', 'HIGH', 'LOW', 'CLOSE', 'PRICE', 'VOLUME', 'Fwd_', 'Date', 'index', 'Daily_Ret']
    ...
    features += ['Entropy_SPY', 'VPIN_Proxy', '0013_CT', '0013_M21_Delta', '0013_Close_Rel_K', 'XGB_Asymmetric_Signal']
    ...
    y_returns = np.column_stack((ml_df['SPY_Fwd_10D_Ret'].values[SEQ_LEN - 1:], 
    ...
    for dom, cols in domain_map.items():
        ...
        base_tensor = torch.tensor(ml_df[cols].shift(1).fillna(0).values, dtype=torch.float32, device=device)
        n, d = base_tensor.shape
        s0, s1 = base_tensor.stride()
        domain_windows_tensor[dom] = torch.as_strided(base_tensor, size=(n - SEQ_LEN + 1, SEQ_LEN, d), stride=(s0, s0, s1))
    ...
    jepa_engine = JepaAttentionEngine(domain_dims=domain_dimensions, latent_dim=16, num_heads=4)
    ...
    for i in range(0, n_samples - train_size, step_size):
        ...
        domain_inputs_train = {dom: domain_windows_tensor[dom][train_idx] for dom in domain_windows_tensor}
        domain_inputs_test = {dom: domain_windows_tensor[dom][test_idx] for dom in domain_windows_tensor}
    ...
    attn_df = pd.DataFrame(out_of_sample_attn, columns=jepa_engine.active_domains, index=out_of_sample_dates)
    ...
    if isinstance(spy_px.columns, pd.MultiIndex): spy_px.columns = spy_px.columns.droplevel(1)
    
    mas = [5, 10, 20, 50, 100, 200]
    for ma in mas:
        spy_px[f'SMA_{ma}'] = spy_px['Close'].rolling(window=ma).mean()
    
    spy_px['Is_Waterfall'] = (spy_px['Close'] < spy_px[[f'SMA_{ma}' for ma in mas]].min(axis=1)) & \
                             (spy_px['Close'].shift(1).rolling(5).max() > spy_px[[f'SMA_{ma}' for ma in mas]].max(axis=1))
    
    results_df.index = pd.to_datetime(results_df.index)
    results_df = results_df.join(spy_px[['Is_Waterfall']].rolling(10).max() == 1.0, how='left')
    results_df.columns = [*results_df.columns[:-1], 'Waterfall_10D_Memory']
    results_df['Waterfall_10D_Memory'] = results_df['Waterfall_10D_Memory'].fillna(False)
    ...
    results_df['VUSTX_Vol'] = results_df['VUSTX_Daily_Ret'].rolling(63).std() * np.sqrt(252)
    results_df['SPY_VUSTX_Corr'] = results_df['SPY_Daily_Ret'].rolling(63).corr(results_df['VUSTX_Daily_Ret'])
    
    # Linear True Risk Parity (Phase 25 Logic)
    # W = (1/sigma) * (1 - rho)
    vustx_budget = (1.0 / (results_df['VUSTX_Vol'] + 1e-4)) * (1.0 - results_df['SPY_VUSTX_Corr'])
    # Normalize budgets if using multiple safe-havens (here just VUSTX)
    results_df['Risk_Parity_Weight'] = vustx_budget.clip(0.5, 1.5) # Minimum 50% defensive budget
    ...
    results_df['VUSTX_Weight'] = (1.0 - results_df['Jepa_Raw_Weight']) * results_df['Risk_Parity_Weight']
    results_df['Cash_Weight'] = 1.0 - results_df['SPY_Weight'] - results_df['VUSTX_Weight']
    results_df['Cash_Weight'] = results_df['Cash_Weight'].clip(0.0, 1.0)
    
    # 1. Bomb Shelter (VIX > 33)
    defense = (results_df['VIX_CLOSE'].shift(1) > 33.0)
    results_df.loc[defense, ['SPY_Weight', 'VUSTX_Weight', 'Cash_Weight']] = [0.0, 0.0, 1.0]
    
    # 2. Waterfall Recovery (Alpha-Bridge)
    # Be aggressive in the recovery phase
    offense = (results_df['VIX_CLOSE'].shift(1) > 33.0) & (results_df['Waterfall_10D_Memory'].shift(1) == True)
    results_df.loc[offense, ['SPY_Weight', 'VUSTX_Weight', 'Cash_Weight']] = [1.2, -0.2, 0.0]
    
    # Final Performance Calculation
    turnover = (results_df['SPY_Weight'].diff().abs() + results_df['VUSTX_Weight'].diff().abs() + results_df['Cash_Weight'].diff().abs()) / 2.0
    results_df['JEPA_Daily_Ret'] = (results_df['SPY_Daily_Ret'] * results_df['SPY_Weight']) + \
    ...
    (1 + results_df[['SPY_Daily_Ret', 'JEPA_Daily_Ret']]).cumprod().plot(ax=ax, color=['#555555', '#ff00ff'])
    ...
    results_df.to_csv('/Users/milocobb/.gemini/antigravity/brain/4b7ec9fe-9cbc-4e19-a3f6-2639cbd15870/jepa_walk_forward_results_19d.csv')
