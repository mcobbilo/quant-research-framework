...
def run_clustering_engine(z_window=252):
    ...
    if z_window == -1:
        print(
            f"[Z-Score Cluster Engine] Computing EXPANDING Percentiles for {len(feature_cols)} Vectors..."
        )
        for col in feature_cols:
            ...
            df[p_col] = (
                ...
                .apply(lambda x: (x <= x[-1]).mean() * 100, raw=True)
            )
            z_score_cols.append(p_col)
    elif z_window == 0:
        print(
            f"[Z-Score Cluster Engine] Computing EXPANDING Exact Z-Scores for {len(feature_cols)} Vectors..."
        )
        for col in feature_cols:
        ...
        print(
            "[Z-Score Cluster Engine] Computing ROLLING 252-Day Heavy-Tail (Student-T df=3) Transformed Z-Scores..."
        )
        for col in feature_cols:
        ...
        print(
            f"[Z-Score Cluster Engine] Computing Rolling {z_window}-Day Exact Gaussian Z-Scores for {len(feature_cols)} Vectors..."
        )
        for col in feature_cols:
    ...
    upside_panic_keywords = [
        "VIX",
        "BAMLC0A0CM",
        "CPC",
        "CPCE",
    ]  # VIX escalating, Credit Spreads blowing out, Puts skyrocketing
    ...
    print(
        f"Max Panic Score recorded in 25 Years: {max_cluster} / {len(feature_cols)} aligning simultaneously"
    )
    print(
        f"Max Euphoria Score recorded in 25 Years: {max_euphoria} / {len(feature_cols)} aligning simultaneously"
    )
    ...
    print(
        f"Found {len(filtered_dates)} Isolated Crash Events where >= 5 Technicals breached 2.5 Sigma thresholds simultaneously.\n"
    )
    ...
    if z_window == -1:
        print(
            " Logic Matrix:       Trigger if >= 5 PANIC arrays hit <1% or >99% Rank simultaneously"
        )
    else:
        print(
            " Logic Matrix:       Trigger if >= 5 PANIC arrays breach 2.5 Sigma simultaneously"
        )
    print(" Exit Protocol:      Hold until Heatmap Score Normalizes (<1)")
    print("")
    print(f" Total Triggers:     {trade_count} Cluster Events")
    print(f" Average Hold:       {avg_hold:.1f} Trading Days")
    print(" Leveraging:         1.0x (No margin) / Yielding 3% Risk-Free in Cash")
    print("")
    print(
        f" [BENCHMARK] SPY Buy/Hold:  ${(10000 * spy_final):,.2f}  Return: {spy_final * 100 - 100:>6.1f}% | Max DD: {spy_max_dd:>6.1f}%"
    )
    print(
        f" [ALGORITHM] Z-Cluster:     ${(10000 * strat_final):,.2f}  Return: {strat_final * 100 - 100:>6.1f}% | Max DD: {strat_max_dd:>6.1f}%"
    )
    print("=======================================================\n")
    ...
    plt.plot(
        df.index,
        df["SPY_Growth"] * 10000,
        label=f"SPY Buy & Hold (Max DD: {spy_max_dd:.1f}%)",
        color="blue",
        alpha=0.6,
    )
    ...
    if z_window == -2:
        plt.plot(
            df.index,
            df["Strat_Growth"] * 10000,
            label=f"Student-T Z-Score Engine (Max DD: {strat_max_dd:.1f}%)",
            color="purple",
            linewidth=2,
        )
        plt.title(
            "Heavy-Tailed Z-Score Engine vs S&P 500 Total Return", fontsize=14, pad=15
        )
        ...
        plt.plot(
            df.index,
            df["Strat_Growth"] * 10000,
            label=f"Gaussian Z-Score Engine (Max DD: {strat_max_dd:.1f}%)",
            color="green",
            linewidth=2,
        )
        plt.title(
            "Classic Gaussian Z-Score Engine vs S&P 500 Total Return",
            fontsize=14,
            pad=15,
        )
