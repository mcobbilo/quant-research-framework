...
def analyze_combinations():
    ...
    out_lines = []
    out_lines.append(f"COMBINATION ANALYSIS: INSIGHT 1 ({var1} {dir1} 50) AND INSIGHT N")
    out_lines.append("="*90)
    ...
    for num in sorted(insights.keys()):
        ...
        if len(combo_df) > 0:
            overlap_count += 1
            out_lines.append(f"Insight 1 + Insight {num} ({varN} {dirN}): {len(combo_df)} Occurrences Found!")
            ...
            out_lines.append(f"  -> 10D Return: {fwd10:+.2f}%  (Win Rate: {w10:>3.0f}%)")
            out_lines.append(f"  -> 20D Return: {fwd20:+.2f}%  (Win Rate: {w20:>3.0f}%)")
            out_lines.append(f"  -> 60D Return: {fwd60:+.2f}%  (Win Rate: {w60:>3.0f}%)")
            out_lines.append(f"  -> Trigger Dates: {', '.join([str(d.date()) for d in combo_df.index])}")
            out_lines.append("-" * 90)
            
    if overlap_count == 0:
        out_lines.append("ZERO simultaneous occurrences detected. Insight 1 (T10YFF) is an isolated temporal anomaly.")
        
    out_path = os.path.join(os.path.dirname(__file__), 'combination_results.txt')
    with open(out_path, 'w') as f:
        f.write('\n'.join(out_lines))
        
    print(f"Intersection sweep complete. Found {overlap_count} overlapping signatures. Output saved to {out_path}.")
