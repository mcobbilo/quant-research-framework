...
def run_ppo_forensics():
    ...
    df['SPY_FWD_3D'] = (df['SPY_CLOSE'].shift(-3) - df['SPY_CLOSE']) / df['SPY_CLOSE'] * 100
    df['SPY_FWD_5D'] = (df['SPY_CLOSE'].shift(-5) - df['SPY_CLOSE']) / df['SPY_CLOSE'] * 100
    df['SPY_FWD_10D'] = (df['SPY_CLOSE'].shift(-10) - df['SPY_CLOSE']) / df['SPY_CLOSE'] * 100
    
    print("[Forensics] Database Loaded. Isolating Top 100 distinct VIX/TNX PPO spikes...")
    ...
    for window in ['3D', '5D', '10D']:
        ...
        print(f" [SPY T+{window}] Average: {avg_ret:+.2f}% | Median: {med_ret:+.2f}% | Win Rate: {win_rate:.1f}%")
