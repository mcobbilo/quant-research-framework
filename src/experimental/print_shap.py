...
def read_shap():
    ...
    model = xgb.XGBRegressor(n_estimators=150, max_depth=4, learning_rate=0.05, subsample=0.8, colsample_bytree=0.8, random_state=42, n_jobs=-1)
    model.fit(X, y)
    ...
    feat_importance = pd.DataFrame({'feature': features, 'shap_importance': mean_abs_shap})
