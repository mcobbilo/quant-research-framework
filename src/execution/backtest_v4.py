...
prices = (
    yf.download(download_tickers, start=START_DATE)["Close"]
    .asfreq("B")
    .ffill()
    .astype(np.float32)
)
...
def create_features(prices, returns, macro):
    ...
    vol_ratio_df = (returns.rolling(21).std() / returns.rolling(252).std()).add_suffix(
        "_vol_ratio"
    )
...
targets = (
    returns.rolling(FORECAST_HORIZON)
    .sum()
    .shift(-FORECAST_HORIZON)
    .add_suffix("_target5d")
    .astype(np.float32)
)
data = pd.concat([features, targets], axis=1)

print("Running SHAP feature elimination on historical data...")
shap_train = data.dropna(
    subset=feature_cols + ["SPY_target5d", "TLT_target5d", "GLD_target5d"]
).head(TRAIN_WINDOW)

shap_xgb_spy = XGBRegressor(
    n_estimators=100,
    learning_rate=0.05,
    max_depth=5,
...
shap_abs_mean = (
    np.abs(shap_values_spy).mean(axis=0)
    + np.abs(shap_values_tlt).mean(axis=0)
    + np.abs(shap_values_gld).mean(axis=0)
) / 3.0
shap_df = pd.DataFrame({"feature": feature_cols, "shap": shap_abs_mean}).sort_values(
    "shap", ascending=False
)
feature_cols = shap_df["feature"].head(150).tolist()
print("✅ Reduced to Top 150 orthogonal features via SHAP.")
...
for h in [1, 2, 3, 5, 10, 21]:
    if h == 5:
        continue
    tdfs.append(
        returns.rolling(h).sum().shift(-h).add_suffix(f"_target{h}d").astype(np.float32)
    )
if tdfs:
...
shuffled_features = np.random.choice(
    feature_cols, size=len(feature_cols), replace=False
).tolist()
chunk_size = max(1, len(shuffled_features) // NUM_SEATS)
seat_features = [
    shuffled_features[i * chunk_size : (i + 1) * chunk_size] for i in range(NUM_SEATS)
]
...
for i in rebalance_idx:
    ...
    for s in range(NUM_SEATS):
        ...
        if train_end_idx_seat < TRAIN_WINDOW:
            continue

        train_slice_s = data.iloc[:train_end_idx_seat].dropna(
            subset=[f"SPY_target{h}d"] + feats
        )
        if len(train_slice_s) < 100:
            continue
        ...
        quar_spy = (
            (np.mean(hit_rates_spy[s][-30:]) < 0.49)
            if len(hit_rates_spy[s]) >= 20
            else False
        )
        quar_tlt = (
            (np.mean(hit_rates_tlt[s][-30:]) < 0.49)
            if len(hit_rates_tlt[s]) >= 20
            else False
        )
        quar_gld = (
            (np.mean(hit_rates_gld[s][-30:]) < 0.49)
            if len(hit_rates_gld[s]) >= 20
            else False
        )
    ...
    pred_history[i] = {
        "spy": current_preds_spy,
        "tlt": current_preds_tlt,
        "gld": current_preds_gld,
    }
...
spy_cum = np.exp(returns["SPY"].loc[start_exec_date:].cumsum()).reindex(
    dates, method="ffill"
)
tlt_cum = np.exp(returns["TLT"].loc[start_exec_date:].cumsum()).reindex(
    dates, method="ffill"
)
gld_cum = np.exp(returns["GLD"].loc[start_exec_date:].cumsum()).reindex(
    dates, method="ffill"
)
...
def calc_metrics(cum):
    ...
    sharpe = (
        rets.mean() / rets.std() * np.sqrt(252 / REBALANCE_DAYS)
        if rets.std() != 0
        else 0
    )
...
print(
    f"Quarantine Rate: {quarantine_counts / max(1, total_seat_evals) * 100:.1f}% of seat-votes were vetoed due to poor moving accuracy."
)
...
print(
    f"{'Year':<6} | {'Strategy%':>9} | {'SPY%':>9} | {'Strat Cum%':>10} | {'SPY Cum%':>10}"
)
print("-" * 55)
for y in yearly_end.index:
    ...
    print(
        f"{y:<6} | {strat_y:>8.2f}% | {spy_y:>8.2f}% | {strat_c:>9.2f}% | {spy_c:>9.2f}%"
    )
...
ax2.fill_between(
    weights_series.index, 0, weights_series * 100, color="green", alpha=0.2
)
