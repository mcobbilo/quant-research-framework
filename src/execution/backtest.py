...
def run_strategy(model, df):
    ...
    for i in range(200, len(trading_days) - 1, 1):
        row = df.iloc[i]
        current_vix = (
            row["VIX"].item() if isinstance(row["VIX"], pd.Series) else row["VIX"]
        )
        ...
        price_start = (
            df["SPY"].iloc[i].item()
            if isinstance(df["SPY"].iloc[i], pd.Series)
            else df["SPY"].iloc[i]
        )
        price_end = (
            df["SPY"].iloc[i + 1].item()
            if isinstance(df["SPY"].iloc[i + 1], pd.Series)
            else df["SPY"].iloc[i + 1]
        )
    ...
    strat_sharpe = (
        (strat_arr.mean() / strat_arr.std()) * np.sqrt(252)
        if strat_arr.std() > 0
        else 0
    )
...
def main():
    ...
    df = df.dropna()
    print("[Backtest] Base data loaded. Calculating technical vectors...")

    df = attach_features(df)
    df = df.dropna()

    spy_start = (
        df["SPY"].iloc[200].item()
        if isinstance(df["SPY"].iloc[200], pd.Series)
        else df["SPY"].iloc[200]
    )
    spy_end = (
        df["SPY"].iloc[-1].item()
        if isinstance(df["SPY"].iloc[-1], pd.Series)
        else df["SPY"].iloc[-1]
    )
    ...
    print(
        f"S&P 500 (Buy & Hold) | Total Return: {spy_total_return:>8,.2f}% | Sharpe Ratio: {spy_sharpe:.2f}"
    )

    for m in models:
        ret, sharpe = run_strategy(m, df)
        print(
            f"{m.name:<20} | Total Return: {ret:>8,.2f}% | Sharpe Ratio: {sharpe:.2f}"
        )
    print("========================================================\n")
