...
def calc_kelly(probability: float, current_vix: float = None) -> float:
    ...
    if probability < 0.0 or probability > 1.0:
        print(f"[OpenAlice Guard] FATAL: Probability {probability} out of bounds (0.0-1.0). Forcing 0.0")
        return 0.0
    ...
    if current_vix is not None:
        ...
        if current_vix < 5.0 or current_vix > 100.0:
            print(f"[OpenAlice Guard] FATAL: VIX ({current_vix}) outside bounds. Forcing 0.0")
            return 0.0
...
class OpenAliceUTA:
    ...
    def execute_order(self, asset: str, action: str, size: float, rationale_hash: str) -> dict:
        """
        ...
        print(f"[OpenAlice] PUSH EXECUTED: Successfully simulated broker execution for commit {commit_id[:8]}.")
...
async def execute_order_activity(params: OrderParams) -> dict:
    ...
    alice = OpenAliceUTA()
    return alice.execute_order(params.asset, params.action, params.size, params.rationale_hash)
...
async def evaluate_market_activity() -> dict:
    ...
    if langgraph_dir != "NEUTRAL":
        if langgraph_dir == torch_decision:
            ...
            print(f"[OpenAlice Guard] ABORT: LangGraph({langgraph_dir}) and TorchTrade({torch_decision}) conflict. Forcing NEUTRAL.")
...
class OpenAliceTradeWorkflow:
    ...
    async def run(self) -> dict:
        ...
        market_eval = await workflow.execute_activity(
            ...
            start_to_close_timeout=timedelta(seconds=60), # Expanded timeout for API calls
        )
        ...
        if decision == "NEUTRAL":
            return {"status": "SKIPPED", "reason": "RL Brain inferred NEUTRAL market matrix. Holding Cash."}
...
async def test_temporal_durable_execution():
    ...
    try:
        ...
        print(f"[Temporal] AI Workflow fully executed and closed successfully!")
