...
logging.basicConfig(
    level=logging.INFO, format="%(asctime)s [%(levelname)s] WikiLinter - %(message)s"
)
...
def lint_vault():
    ...
    if empty_files:
        logging.warning(
            f"Found {len(empty_files)} stub/empty nodes. Queuing LLM Imputation cycle for Web Search."
        )
    else:
