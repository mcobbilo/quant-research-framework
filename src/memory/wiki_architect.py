...
logging.basicConfig(
    level=logging.INFO, format="%(asctime)s [%(levelname)s] WikiArchitect - %(message)s"
)

RAW_DIR = os.path.abspath(
    os.path.join(os.path.dirname(__file__), "../../obsidian_vault/raw")
)
PACKETS_DIR = os.path.abspath(
    os.path.join(os.path.dirname(__file__), "../../obsidian_vault/packets")
)
...
class WikiArchitect:
    ...
    def scan_raw(self):
        ...
        new_files = [
            f for f in files if f not in self.processed_files and os.path.isfile(f)
        ]
        return new_files
