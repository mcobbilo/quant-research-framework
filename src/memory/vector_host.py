...
class VectorHost:
    ...
    def __init__(
        self,
        data_path: str = "data/chroma",
        collection_name: str = "council_of_winners",
    ):
    ...
    def cache_insight(
        ...
        self.collection.add(documents=[insight], metadatas=[meta], ids=[doc_id])
        return doc_id

    def retrieve_similar_regimes(
        self, query: str, regime: str, n_results: int = 3
    ) -> List[Dict[str, Any]]:
        """
        ...
        payload = []
        for doc_arr, meta_arr, dist_arr in zip(
            results["documents"], results["metadatas"], results["distances"]
        ):
            for doc, meta, distance in zip(doc_arr, meta_arr, dist_arr):
                payload.append(
                    {"insight": doc, "metadata": meta, "similarity_distance": distance}
                )
