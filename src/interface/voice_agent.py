...
async def run_live():
    ...
    try:
        async with client.aio.live.connect(
            model=resolved_model, config=config
        ) as session:
            print("[Voice Agent] WebSockets synchronized. The Swarm is listening! 🟢\n")
                            ...
                            if tool_call is not None:
                                for func_call in tool_call.function_calls:
                                    ...
                                    if func_call.name == "get_current_market_status":
                                        tool_result = {
                                            "status": "The VIX is stable at 14.5. No tail-risk. Target exposure 1.0x SPY."
                                        }
                                            ...
                                            function_responses=[
                                                {
                                                    ...
                                                    "name": func_call.name,
                                                    "response": {
                                                        "error": "Tool not implemented globally."
                                                    },
                                                }
                                            ]
                                        )
