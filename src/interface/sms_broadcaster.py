...
def send_text_alert(
    target_allocation, spy_price, crash_prob, shap_drivers, phone_number="8172879263"
):
    """
    ...
    print(
        f"[SMS Engine] Compiling AppleScript payload to target cell number {phone_number}..."
    )
    ...
    try:
        subprocess.run(
            ["osascript", "-e", applescript, phone_number, text_body],
            check=True,
            capture_output=True,
            text=True,
        )
        print(
            f"[SMS Engine] SUCCESS: Native macOS Text Alert safely dispatched payload to {phone_number}."
        )
