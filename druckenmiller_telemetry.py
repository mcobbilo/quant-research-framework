...
def main():
    ...
    df_fred["Core_CPI_NSA_YoY"] = (
        df_fred["Core_CPI_NSA"].pct_change(252) * 100
    )  # approx 1 yr trading days since it's daily ffilled

    # Taylor Rule = CPI_NSA_YoY + 2.0 (neutral) + 0.5*(CPI_NSA_YoY - 2.0) - 0.5*(UNRATE_NSA - 4.5)
    df_fred["Taylor_Rule_Rate"] = (
        df_fred["Core_CPI_NSA_YoY"]
        + 2.0
        + 0.5 * (df_fred["Core_CPI_NSA_YoY"] - 2.0)
        - 0.5 * (df_fred["Unemployment_NSA"] - 4.5)
    )
    ...
    df_fred["net_liquidity_momentum"] = df_fred["Net_Liquidity"].pct_change(
        65
    )  # 13 weeks * 5 days = 65 trading days
    ...
    df_fred["hy_spread_velocity"] = df_fred["HY_Spread"].pct_change(
        21
    )  # 1 month velocity
    ...
    # ---------------------------------------------------------
    shift_cols = [
        "taylor_policy_spread",
        "us2y_ffr_divergence",
        "net_liquidity_momentum",
        "shadow_liquidity_momentum",
        "hy_spread_velocity",
        "ccc_bb_spread_dispersion",
        "t_bill_drain_ratio",
    ]
    ...
    df_final = df_final.dropna(
        subset=["taylor_policy_spread", "shadow_liquidity_momentum"]
    )
