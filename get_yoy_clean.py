...
final_ret, mdd, df_cont = run_xgboost_audit(
    seed=0, noise_std=0.0, guillotine=False, test_name="YEARLY_CALC_10Y"
)

df_cont["year"] = df_cont["ds"].dt.year
print("\n=== Year-by-Year Performance (Baseline 10-Year) ===")
print(
    f"{'Year':<6} | {'Model Return':<14} | {'S&P 500 Return':<14} | {'Model Max DD':<14}"
)
print("-" * 55)
...
for year, group in df_cont.groupby("year"):
    ...
    print(
        f"{year:<6} | {comp_strat * 100:>13.2f}% | {comp_spy * 100:>13.2f}% | {dd * 100:>13.2f}%"
    )
