...
try:
    ...
    print(
        "[CouncilVoice] Warning: 'sphn' module not found. Voice functionality may be limited."
    )
    sphn = None
...
class CouncilVoiceOrchestrator:
    def __init__(self, framework_path, hf_token=None):
        ...
        self.hf_token = (
            hf_token or env_vars.get("HF_TOKEN") or os.environ.get("HF_TOKEN")
        )
    ...
    def speak_fast(self, agent_name, text):
        ...
        output_wav = os.path.join(
            self.audio_logs_dir, f"fast_{agent_name}_{timestamp}.mp3"
        )
    ...
    def create_silence_wav(self, duration, output_path, sample_rate=24000):
        ...
        with wave.open(output_path, "wb") as wf:
            ...
            silence_data = np.zeros(
                int(duration * sample_rate), dtype=np.int16
            ).tobytes()
    ...
    def speak(self, agent_name, text, duration_estimation_factor=0.15, use_fast=False):
        ...
        if agent_name not in self.agent_voices:
            print(
                f"[CouncilVoice] Warning: Unknown agent {agent_name}. Using NATF1.pt as default."
            )
            voice_pt = "NATF1.pt"
        else:
        ...
        input_wav = os.path.join(
            self.audio_logs_dir, f"drive_{agent_name}_{timestamp}.wav"
        )
        output_wav = os.path.join(self.audio_logs_dir, f"{agent_name}_{timestamp}.wav")
        output_text = os.path.join(
            self.audio_logs_dir, f"{agent_name}_{timestamp}.json"
        )
        ...
        cmd = [
            ...
            text,
            "--device",
            "cpu",  # Explicitly use CPU for Mac stability
        ...
        print(
            f"[CouncilVoice] Generating speech for {agent_name} ({duration}s drive)..."
        )
...
if __name__ == "__main__":
    ...
    orch.speak(
        "Beta",
        "This is the Risk Manager. Your hypothesis has significant lookahead bias. Abort immediately.",
    )
