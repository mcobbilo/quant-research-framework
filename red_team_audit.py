...
def run_audit(seed=42, noise_std=0.0, guillotine=False, test_name=""):
    ...
    if noise_std > 0:
        ...
        for col in hist_exog:
            ...
            df_panel[col] = df_panel[col] + np.random.normal(0, std_dev * noise_std, len(df_panel))
    ...
    model = xLSTMForecast(
        ...
        hist_exog_list=hist_exog, freq='B', random_seed=seed
    )
    ...
    print(f"[{test_name}] COMPLETE | Asset Yield: {base_ret:.2f}x | Model Yield: {final_ret:.2f}x | Max DD: {mdd:.2%}")
...
if __name__ == "__main__":
    ...
    run_audit(seed=1337, noise_std=0.0, guillotine=False, test_name="SEED_1337_PERTURBATION")
    run_audit(seed=42, noise_std=0.05, guillotine=False, test_name="5PCT_NOISE_INJECTION")
    run_audit(seed=42, noise_std=0.0, guillotine=True, test_name="DATA_GUILLOTINE_TIMETRAVEL")
