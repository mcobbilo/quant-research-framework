...
def run_extended_backtest():
    ...
    spy_fresh = yf.download(
        "SPY", start=max_date.strftime("%Y-%m-%d"), end="2026-04-11"
    )["Close"]
    ...
    print(
        f"{eval_df['Date'].iloc[0].strftime('%Y-%m-%d')}: Initial -> {pos_map.get(eval_df['Position'].iloc[0])}"
    )
    for _, row in changes.iterrows():
        print(
            f"{row['Date'].strftime('%Y-%m-%d')}: Switched to {pos_map.get(row['Position'])}"
        )
