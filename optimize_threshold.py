...
for t in thresholds:
    ...
    df_cont["asset_return"] = (df_cont["SPY_PRICEMAP"] - df_cont["y_prev"]) / df_cont[
        "y_prev"
    ]

    df_cont = pd.merge(
        df_cont,
        df_pred[["signal_date", "prob_up"]],
        left_on="ds",
    ...
    print(
        f"{t:.2f}      | {final_ret:>10.2f}x | {mdd:>6.2%} | {spy_exp:>12.1%} | {vustx_exp:>10.1%} | {cash_exp:>9.1%}"
    )
