...
def build_phase10_engine():
    ...
    # ---------------------------------------------------------
    df_core['y_cls_45d'] = (df_core['spy_close'].shift(-HORIZON) > df_core['spy_close']).astype(int)
    df_core['y_reg_45d'] = (df_core['spy_close'].shift(-HORIZON) - df_core['spy_close']) / df_core['spy_close']
    
    # Inverse Correlation Trackers (Rolling 63-day)
    spy_ret = df_core['spy_close'].pct_change()
    df_core['corr_spy_vustx'] = spy_ret.rolling(63).corr(df_core['vustx_close'].pct_change())
    df_core['corr_spy_gld'] = spy_ret.rolling(63).corr(df_core['gld_close'].pct_change())
    
    exclude_cols = ['ds', 'Date', 'spy_close', 'vustx_close', 'gld_close', 'vix_close', 'PRICEMAP', 'unique_id', 'y_cls_45d', 'y_reg_45d', 'corr_spy_vustx', 'corr_spy_gld']
    num_features = [c for c in df_core.columns if c not in exclude_cols]
    num_features = df_core[num_features].select_dtypes(include=[np.number]).columns.tolist()
    ...
    print(f"Extracting top 60 DNA Features via L1 Regularization...")
    lr = LogisticRegression(penalty='l1', solver='liblinear', C=1.0, random_state=42, max_iter=200)
    lr.fit(X_scaled, y_target)
    
    feature_importances = pd.DataFrame({'feature': num_features, 'importance': np.abs(lr.coef_[0])})
    ...
    for i in tqdm(range(total_steps)):
        ...
        df_train['regime'] = (df_train['regime'] == danger_regime).astype(int) # 1 = Danger, 0 = Safe
        ...
        # ---------------------------------------------------------
        clf_args = dict(n_estimators=100, max_depth=4, learning_rate=0.03, subsample=0.8, eval_metric='logloss')
        reg_args = dict(n_estimators=100, max_depth=4, learning_rate=0.03, subsample=0.8)
    ...
    for idx, row in df_cont.iterrows():
        ...
        # Regime 1 (Danger) -> Enforce Asymmetric Rotation
            ...
            if row['pred_magnitude'] < -row['conformal_buffer']:
                ...
                if row['spy_vustx_corr'] < row['spy_gld_corr'] and row['spy_vustx_corr'] < -0.1:
                    alloc.append('VUSTX')
                    rets.append(row['vustx_ret'])
                elif row['spy_gld_corr'] < row['spy_vustx_corr'] and row['spy_gld_corr'] < -0.1:
                    alloc.append('GLD')
