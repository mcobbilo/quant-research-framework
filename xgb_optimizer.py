...
def optimize_xgb():
    ...
    df_core["y_vustx"] = (
        df_core["vustx_close"].shift(-HORIZON) > df_core["vustx_close"]
    ).astype(int)
    df_core["y_gld"] = (
        df_core["gld_close"].shift(-HORIZON) > df_core["gld_close"]
    ).astype(int)
    df_core["y_vix"] = (
        df_core["vix_close"].shift(-HORIZON) > df_core["vix_close"]
    ).astype(int)
    ...
    exclude_cols = [
        ...
        "PRICEMAP",
        "unique_id",
    ] + [c for c in df_core.columns if c.startswith("y_")]

    all_features = [c for c in df_core.columns if c not in exclude_cols]
    all_features = (
        df_core[all_features].select_dtypes(include=[np.number]).columns.tolist()
    )
    ...
    print(
        f"Running L1 Logistic Regression over {len(all_features)} numeric features..."
    )
    ...
    feature_importances = pd.DataFrame(
        {"feature": all_features, "importance": coef_abs}
    )
