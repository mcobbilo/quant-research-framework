...
def extract_subgraph(file_path):
    ...
    top_files = sorted([n for n in nodes if n.get('type') == 'file'], key=lambda x: x.get('centrality', 0), reverse=True)[:15]
    top_papers = sorted([n for n in nodes if n.get('type') == 'paper'], key=lambda x: x.get('centrality', 0), reverse=True)[:10]
    
    selected_node_ids = set([n['id'] for n in hubs + top_files + top_papers])
    
    # 3. Find edges between selected nodes
    subgraph_edges = [e for e in edges if e['source'] in selected_node_ids and e['target'] in selected_node_ids]
    ...
    for concept_id in concepts:
        concept_edges = [e for e in edges if e['source'] == concept_id or e['target'] == concept_id]
    ...
    for n in final_nodes:
        ...
        safe_id = nid.replace('://', '_').replace('.', '_').replace('/', '_').replace(' ', '_').replace('-', '_').replace('>', 'GT')
        label = nid.split('/')[-1]
    ...
    for e in unique_edges:
        source_id = e['source'].replace('://', '_').replace('.', '_').replace('/', '_').replace(' ', '_').replace('-', '_').replace('>', 'GT')
        target_id = e['target'].replace('://', '_').replace('.', '_').replace('/', '_').replace(' ', '_').replace('-', '_').replace('>', 'GT')
