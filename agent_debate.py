...
def simulate_debate():
    ...
    conversation_history = [
        {
            "role": "system",
            "content": "You are coordinating a 3-way debate between Alpha, Beta, and Gamma. You must generate 1 turn at a time.",
        }
    ]
    ...
    rounds = [
        (
            "Alpha",
            "Initiate the debate. Why is Moirai 2.0 / TSFM superior for institutional alpha?",
        ),
        (
            "Beta",
            "Counter Alpha. Why are TSFMs risky 'black boxes' compared to hardened XGBoost ensembles?",
        ),
        (
            "Gamma",
            "Intervene. Why is TFT (Temporal Fusion Transformer) actually the structural sweet spot for both regime detection and time series memory?",
        ),
        (
            "Alpha",
            "Rebuttal: 'Data is the new static'. Why is zero-shot scaling unbeatable in non-stationary markets?",
        ),
        (
            "Beta",
            "Final Warning: Why will 'Vibe Coding' without explainable loss functions lead to catastrophic drawdowns?",
        ),
        (
            "Gamma",
            "The Conclusion: Synthesize a hybrid path forward using TFT as the backbone.",
        ),
    ]
    ...
    for agent, prompt in rounds:
        ...
        conversation_history.append(
            {"role": "user", "name": agent, "content": context_prompt}
        )
        ...
        conversation_history.append(
            {"role": "assistant", "name": agent, "content": response}
        )
