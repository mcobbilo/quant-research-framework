...
if any(forbidden in db_path.lower() for forbidden in ['..', '/etc', '/var', 'root', 'sys']) or db_path.startswith('/'):
...
feature_list = ['VIX_CLOSE','MOVE_CLOSE','VIX_TNX_RATIO','SPY_PCT_FROM_200','SPY_PPO_HIST',
                'VIX_TERM_STRUCTURE_3M','SKEW_CLOSE','SPY_ATR_PCT','SPY_VOL_RATIO_21_252',
                'CORR_SPY_VUSTX_63_ZSCORE','VIX_VVIX_RATIO_Z','SPY_ACCEL_MOM','VIX_SMA20',
                'VIX_BB_WIDTH','SPY_PCT_FROM_DONCHIAN_UPPER','T10Y2Y','TEDRATE','NFCI']
feature_cols = [f for f in feature_list if f in df.columns]
if len(feature_cols) < 5:
    feature_cols = [c for c in df.select_dtypes(include=[np.number]).columns.tolist() 
                    if c not in ['RETURN','TARGET']][:15]
...
for i, gamma in enumerate(gammas):
    ...
    model = xgb.train({'max_depth': 4, 'eta': 0.08, 'subsample': 0.8, 'colsample_bytree': 0.8, 'seed': 42},
                      dtrain, num_boost_round=60, obj=make_obj(), verbose_eval=False)
    models.append(model)
...
sharpe = (test_df['STRAT_RETURN'].mean() / std_ret * np.sqrt(252)) if std_ret > 1e-8 else 0.0
