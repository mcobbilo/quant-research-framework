...
def generate_treasury_chart():
    ...
    pivot['T_Bill_Percentage'] = (pivot['Bills_Trillions'] / (pivot['Bills_Trillions'] + pivot['Notes_Bonds_Trillions'])) * 100
    ...
    merged = pd.merge(fred_df[['RRP_Trillions']], pivot[['Bills_Trillions', 'T_Bill_Percentage']], left_index=True, right_index=True, how='left')
    ...
    fig, (ax1, ax3) = plt.subplots(2, 1, figsize=(14, 12), gridspec_kw={'height_ratios': [2, 1]}, sharex=True)
    
    # --- Top Panel: Cross-Correlation of T-Bills vs RRP Drain ---
    # Plot T-Bills Outstanding (Left Axis)
    ax1.plot(merged.index, merged['Bills_Trillions'], color='#00d2ff', linewidth=3, label="T-Bills Outstanding ($ Trillions)")
    ax1.fill_between(merged.index, merged['Bills_Trillions'], min(merged['Bills_Trillions'])*.9, color='#00d2ff', alpha=0.1)
    ax1.set_ylabel("U.S. T-Bills Outstanding ($ Trillions)", fontsize=12, color='#00d2ff')
    ax1.tick_params(axis='y', labelcolor='#00d2ff')
    ax1.grid(color='#2A3459', linestyle='--', alpha=0.7)
    
    # Plot Reverse Repo Drain (Right Axis)
    ax2 = ax1.twinx()
    ax2.plot(merged.index, merged['RRP_Trillions'], color='#ef5350', linewidth=3, label="Reverse Repo Facility (RRP)")
    ax2.fill_between(merged.index, merged['RRP_Trillions'], 0, color='#ef5350', alpha=0.1)
    ax2.set_ylabel("Fed RRP Balance ($ Trillions)", fontsize=12, color='#ef5350')
    ax2.tick_params(axis='y', labelcolor='#ef5350')
    
    ax1.set_title('The Treasury "Stealth QE": Replacing Lost Fed Liquidity (5-Year Window)', fontsize=16, color='white', pad=20)
    
    # Combine Legends
    lines_1, labels_1 = ax1.get_legend_handles_labels()
    lines_2, labels_2 = ax2.get_legend_handles_labels()
    ax1.legend(lines_1 + lines_2, labels_1 + labels_2, loc='upper left', frameon=True, fontsize=12, facecolor='#0D1117')

    # --- Bottom Panel: T-Bill Debt Percentage ---
    ax3.plot(merged.index, merged['T_Bill_Percentage'], color='#26a69a', linewidth=2)
    ax3.fill_between(merged.index, merged['T_Bill_Percentage'], 0, color='#26a69a', alpha=0.2)
    ax3.axhline(20.0, color='yellow', linestyle='--', linewidth=1.5, label="TBAC Guideline Max Limit (20%)")
    ax3.set_ylabel("T-Bills as % of Total Marketable Debt", fontsize=12, color='white')
    ax3.grid(color='#2A3459', linestyle='--', alpha=0.7)
    ax3.set_ylim(min(merged['T_Bill_Percentage']) - 1, max(merged['T_Bill_Percentage']) + 2)
