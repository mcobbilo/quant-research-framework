...
LIQUIDITY_KEYWORDS = [
    ...
    "target range", "federal funds rate", "basis points", "bps", "hike", "raise", 
    "increase", "cut", "lower", "decrease", "accommodative", "restrictive", "terminal rate",
    # 2. Balance Sheet Operations
    "balance sheet", "asset purchases", "taper", "tapering", "quantitative easing", "qe",
    "runoff", "roll off", "reinvestment", "mortgage-backed securities", "mbs",
    # 3. Forward Guidance & Signaling
    "patient", "measured pace", "gradual", "data-dependent", "transitory",
    "dot plot", "summary of economic projections", "sep", "forward guidance", "appropriate",
    # 4. Dual Mandate & Economic Assessment
    "price stability", "maximum employment", "inflation expectations", "pce",
    "labor market conditions", "headwinds", "crosswinds",
    # 5. Consensus & Dissent
    "unanimous", "dissented", "dissenting", "preferred to", "upside risk", "downside risk",
    "some participants", "many participants", "most participants"
]
...
def evaluate_with_agent(title: str, description: str) -> dict:
    ...
    system_prompt = f"""
    You are an institutional quantitative macro analyst. Analyze the following Fed press release:
    
    TITLE: {title}
    DESCRIPTION: {description}
    
    Output exactly valid JSON containing:
    1. is_major_liquidity_event: true/false
    2. directional_impact: float (-1.0 for severe contraction to 1.0 for massive easing/printing)
    3. surprise_factor: float (0.0 for heavily telegraphed to 1.0 for emergency shock)
    """
