...
def run_xgboost_audit(seed=0, noise_std=0.0, guillotine=False, test_name="BASELINE"):
    ...
    # Dynamic Doctor Copper Z-Score Logic
    copper_str = df_returns['hg_close_prev'] / df_returns['hg_sma200_prev']
    copper_str_mean = copper_str.rolling(504).mean()
    copper_str_std = copper_str.rolling(504).std()
    df_returns['copper_zscore'] = (copper_str - copper_str_mean) / copper_str_std
    df_returns['copper_zscore'].fillna(method='ffill', inplace=True)
    df_returns['copper_zscore'].fillna(method='bfill', inplace=True)
    ...
    for idx, row in df_cont.iterrows():
        ...
        copper_z = row['copper_zscore']
        
        # Doctor Copper Defensive Block (Dynamic Z-Score)
        dr_copper_veto = (copper_z < -0.5)
