...
def run_xgboost_audit(seed=0, noise_std=0.0, guillotine=False, test_name="BASELINE"):
    ...
    query = """
        SELECT Date as ds, 
               SPY_OPEN as spy_open, VUSTX_OPEN as vustx_open, GLD_OPEN as gld_open, VIX_OPEN as vix_open,
    ...
    # ---------------------------------------------------------
    # MULTI-ASSET MICROSTRUCTURE TELEMETRY INJECTION
    # ---------------------------------------------------------
    df_core['spy_intraday_mom'] = (df_core['spy_close'] - df_core['spy_open']) / df_core['spy_open']
    df_core['vustx_intraday_mom'] = (df_core['vustx_close'] - df_core['vustx_open']) / df_core['vustx_open']
    df_core['gld_intraday_mom'] = (df_core['gld_close'] - df_core['gld_open']) / df_core['gld_open']
    df_core['vix_intraday_mom'] = (df_core['vix_close'] - df_core['vix_open']) / df_core['vix_open']
    
    # Synthetics
    df_core['telemetry_regime_divergence'] = df_core['vustx_intraday_mom'] - df_core['spy_intraday_mom']
    df_core['vol_equity_divergence'] = df_core['vix_intraday_mom'] + df_core['spy_intraday_mom']
    
    df_core.fillna(method='bfill', inplace=True)
    ...
    features = [
        ...
        'vix_vvix_ratio_z', 'sent_crossasset_vol_ratio', 'sent_0dte_repression_z', 'vix_vxv_ratio_z',
        'spy_vol_ratio_21_252', 'spy_accel_mom', 'spy_atr_14', 'spy_atr_pct', 'market_breadth_zscore_252d',
        'vix_to_10y_mom21', 'corr_spy_vustx_63', 'skew_zscore_252',
        'spy_intraday_mom', 'vustx_intraday_mom', 'gld_intraday_mom', 'vix_intraday_mom',
        'telemetry_regime_divergence', 'vol_equity_divergence'
    ]
    ...
    df_core['y_vix'] = np.where(df_core['vix_close'].shift(-HORIZON) > df_core['vix_close'], 1, 0)
