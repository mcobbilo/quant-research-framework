...
def run_xgboost_audit(seed=0, noise_std=0.0, guillotine=False, test_name="BASELINE"):
    ...
    # ---------------------------------------------------------
    df_core["spy_intraday_mom"] = (
        df_core["spy_close"] - df_core["spy_open"]
    ) / df_core["spy_open"]
    df_core["vustx_intraday_mom"] = (
        df_core["vustx_close"] - df_core["vustx_open"]
    ) / df_core["vustx_open"]
    df_core["gld_intraday_mom"] = (
        df_core["gld_close"] - df_core["gld_open"]
    ) / df_core["gld_open"]
    df_core["vix_intraday_mom"] = (
        df_core["vix_close"] - df_core["vix_open"]
    ) / df_core["vix_open"]

    # Synthetics
    df_core["telemetry_regime_divergence"] = (
        df_core["vustx_intraday_mom"] - df_core["spy_intraday_mom"]
    )
    df_core["vol_equity_divergence"] = (
        df_core["vix_intraday_mom"] + df_core["spy_intraday_mom"]
    )
    ...
    features = [
        ...
        "gld_intraday_mom",
        "vix_intraday_mom",
        "telemetry_regime_divergence",
        "vol_equity_divergence",
    ]
