...
def run_full_backtest():
    ...
    for _, row in changes.iterrows():
        ...
        rationale = []
        if liqi < -0.05:
            rationale.append("Defensive: Severe Global Liquidity Drain (<-5%)")
        elif liqi > 0.05:
            rationale.append(
                "Offensive: Explosive Central Bank Liquidity Growth (>+5%)"
            )

        if cpi > 0.04 and fede < cpi:
            rationale.append("Caution: Runaway Inflation with Fed Behind-the-Curve")
        elif fomc < -0.2:
            rationale.append("Defensive: Ultra-Hawkish FOMC NLP Paradigm Shift")
        elif fomc > 0.2:
            rationale.append("Offensive: Dovish/Accommodative FOMC Communications")

        if not rationale:
            rationale.append(
                "Standard Model Re-evaluation (Momentum / Volatility Bounds Crossed Thresholds)"
            )
