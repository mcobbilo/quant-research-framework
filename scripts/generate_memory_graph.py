...
class GraphMemoryGenerator:
    def __init__(self, root_dir, research_dir):
        ...
        self.concepts = ["VIX Scaling", "Kelly Criterion", "Regime Detection", "GMM", "TFT", "Non-Leveraged", "Counter-Cyclical", "Recurrent Memory", "VIX > 40", "MOVE > 130", "Dead Zone", "Stubbing", "Execution Safety"]
        ...
        for root, _, files in os.walk(self.research_dir):
            for file in files:
                if file.endswith((".md", ".txt")):
                    ...
                    if any(x in rel_path for x in ["personaplex", "tmp_", "build/", "venv"]):
                        continue
            ...
            for file in files:
                if file.endswith(".md"):
                    ...
                    try:
                        ...
                        self.graph.add_node(node_id, type="memory", category="ground_truth", weight=2.0)
        ...
        for root, _, files in os.walk(self.obsidian_dir):
            for file in files:
                if file.endswith(".md"):
                    ...
                    try:
                        self.graph.add_node(node_id, type="doctrine", category="ground_truth")
                        with open(path, "r", encoding="utf-8") as f:
                            content = f.read()
                            self._link_doc_concepts(node_id, content)
                    except Exception as e:
    ...
    def synthesize(self):
        ...
        with open(report_path, "w") as f:
            ...
            sorted_nodes = sorted(self.graph.nodes(data=True), key=lambda x: x[1].get('centrality', 0), reverse=True)
            for node, attr in sorted_nodes[:10]:
                f.write(f"- **{node}** (Type: {attr.get('type')}, Score: {attr.get('centrality'):.3f})\n")
