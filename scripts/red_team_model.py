...
from src.execution.backtest import AgenticStrategy, attach_features
...
class RedTeamAgenticStrategy(AgenticStrategy):
    ...
    def evaluate(self, row):
        ...
        (self.action_history[-1] if self.action_history else 0.5)
        ...
        consensus_score = (
            self.consensus_override if self.consensus_override is not None else 0.92
        )
...
def main():
    ...
    results.append(
        {"Case": "5% Feature Noise", "Return": ret, "Sharpe": shp, "MDD": mdd}
    )
    ...
    results.append(
        {"Case": "VIX Hammer (2021 Spike)", "Return": ret, "Sharpe": shp, "MDD": mdd}
    )
    ...
    results.append(
        {"Case": "Low Consensus (0.50)", "Return": ret, "Sharpe": shp, "MDD": mdd}
    )
    ...
    if noise_ret < baseline_ret * 0.3:
        print(
            "[RED TEAM] WARNING: High Fragility detected. Noise collapsed alpha by >70%."
        )
    else:
    ...
    if results[4]["Return"] > 0:
        print(
            "[RED TEAM] WARNING: Failure to Fallback. Model trading through Low Consensus."
        )
    else:
