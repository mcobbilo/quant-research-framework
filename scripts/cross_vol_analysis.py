...
def calculate_cross_vol_returns():
    ...
    data = yf.download(tickers, start="2000-01-01", end="2025-01-01", progress=False, auto_adjust=True)
    
    # We use Close and ffill/dropna to align all 4 series
    df = data["Close"].ffill().dropna()
    
    print(f"[Research] Analyzing synchronized period: {df.index[0].date()} to {df.index[-1].date()}")
