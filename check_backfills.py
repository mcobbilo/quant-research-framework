...
df = pd.read_csv('/Users/milocobb/Desktop/Recent Swarm Papers/quant_framework/market_data_inspection.csv')
report_lines = []
...
artifact_path = os.path.expanduser("~/.gemini/antigravity/brain/86f8d6d6-545f-43de-8268-7b50b6d1c47a/backfill_analysis.md")
with open(artifact_path, 'w') as f:
    f.write("# Data Integrity: Backfill Analysis\n")
    f.write("This table shows the true start dates for all features in the dataset. Variables with hundreds or thousands of 'Constant Days' at the beginning were heavily backward-filled with bogus data due to `pandas.bfill()`.\n\n")
    ...
    for row in report_lines:
        if row['Constant_Days'] > 20:  # Only show significant backfills
            f.write(f"| **{row['Feature']}** | {row['True_Start_Date']} | {row['Constant_Days']} Trading Days |\n")
