...
results = []
print("SPY_Thresh | VIX_Thresh | Strat Return | Max DD | SPY Exposure | VUSTX Expo | GLD Expo | CASH Expo")
print("-" * 100)
...
for s_t in spy_thresholds:
    for v_t in vix_thresholds:
        ...
        results.append({
            'spy_t': s_t, 'vix_t': v_t, 'ret': final_ret, 'mdd': mdd,
            'spy': spy_exp, 'vustx': vustx_exp, 'gld': gld_exp, 'cash': cash_exp
        })
        
        print(f"{s_t:.2f}       | {v_t:.2f}       | {final_ret:>10.2f}x | {mdd:>6.2%} | {spy_exp:>12.1%} | {vustx_exp:>10.1%} | {gld_exp:>8.1%} | {cash_exp:>9.1%}")
